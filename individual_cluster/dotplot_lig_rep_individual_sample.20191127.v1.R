...
# plot dotplot by sample with all the ligands and receptors--------------------------------------------------
for (snRNA_aliquot_id_tmp in snRNA_aliquot_ids) {
  ...
  dir.create(path = dir_out_aliquot)
  
  file2write <- paste(dir_out_aliquot, snRNA_aliquot_id_tmp,".Individual_Clustered.", "LigandReceptor.Dotplot.", run_id, ".png", sep="")
  ...
  ## output lineplot by cell type
  dir_out_celltype <- paste0(dir_out_aliquot, "Lineplot_By_Pair_Exp_Cell_Type/")
  dir.create(path = dir_out_celltype)
  ...
  for (pair_type_tmp in unique(lig_rep_tab2plot$pair_by_celltype_exp)) {
    ...
    file2write <- paste(dir_out_celltype, snRNA_aliquot_id_tmp,".Individual_Clustered.", "LigandReceptor.", pair_type_tmp, ".Dotplot.", run_id, ".png", sep="")
    png(file2write, width = 2500, height = 3000, res = 150)
  ...
  ## output lineplot by ligand
  dir_out_ligand <- paste0(dir_out_aliquot, "Lineplot_By_Ligand/")
  dir.create(path = dir_out_ligand)
  
  for (ligand_tmp in unique(lig_rep_tab2plot$Ligand.ApprovedSymbol)) {
    p <- ggplot()
    p <- p + geom_line(data = line_plot_tab[grepl(x = line_plot_tab$Pair.Name, pattern = paste0(ligand_tmp, "_")),], mapping = aes(x = x, y = y, group = Pair.Name), color = line_colors[pair_type_tmp])
    p <- p + geom_text(data = line_plot_tab[line_plot_tab$gene_type == "Ligand",], mapping = aes(x = x, y = y, label = value), angle = 90, nudge_y = 0.05)
    p <- p + geom_text(data = line_plot_tab[line_plot_tab$gene_type == "Receptor",], mapping = aes(x = x, y = y, label = value), angle = 90, nudge_y = -0.05)
    p <- p + scale_x_continuous(expand = c(0.01,0.01), limits=range(line_plot_tab$x))
    p <- p + theme_bw()
    ...
    file2write <- paste(dir_out_ligand, snRNA_aliquot_id_tmp,".Individual_Clustered.", ligand_tmp, ".Ligand.Dotplot.", run_id, ".png", sep="")
    png(file2write, width = 2500, height = 3000, res = 150)
    p <- plot_grid(ligand_exp_plot, line_plot_tmp, receptor_exp_plot, 
                   nrow = 3, rel_heights = c(1, 1, 1), align = "v", axis = "lr", greedy = F)
    print(p)
  ...
  ## output lineplot by ligand
  dir_out_receptor <- paste0(dir_out_aliquot, "Lineplot_By_Receptor/")
  dir.create(path = dir_out_receptor)
  
  for (receptor_tmp in unique(lig_rep_tab2plot$Receptor.ApprovedSymbol)) {
    p <- ggplot()
    p <- p + geom_line(data = line_plot_tab[grepl(x = line_plot_tab$Pair.Name, pattern = paste0("_", receptor_tmp)),], mapping = aes(x = x, y = y, group = Pair.Name), color = line_colors[pair_type_tmp])
    p <- p + geom_text(data = line_plot_tab[line_plot_tab$gene_type == "Ligand",], mapping = aes(x = x, y = y, label = value), angle = 90, nudge_y = 0.05)
    p <- p + geom_text(data = line_plot_tab[line_plot_tab$gene_type == "Receptor",], mapping = aes(x = x, y = y, label = value), angle = 90, nudge_y = -0.05)
    p <- p + scale_x_continuous(expand = c(0.01,0.01), limits=range(line_plot_tab$x))
    p <- p + theme_bw()
    ...
    file2write <- paste(dir_out_receptor, snRNA_aliquot_id_tmp,".Individual_Clustered.", receptor_tmp, ".Receptor.Dotplot.", run_id, ".png", sep="")
    png(file2write, width = 2500, height = 3000, res = 150)
    p <- plot_grid(ligand_exp_plot, line_plot_tmp, receptor_exp_plot, 
                   nrow = 3, rel_heights = c(1, 1, 1), align = "v", axis = "lr", greedy = F)
    print(p)
