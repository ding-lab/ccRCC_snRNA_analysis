...
## spcify assay
assay_process <- "RNA"
cat(paste0("Will extract assay: ", assay_process, "\n"))
...
srat@meta.data$Cell_group.shorter <- mapvalues(x = srat@meta.data$id_cell, from = barcode2celltype_df$id_cell, to = as.vector(barcode2celltype_df$Cell_group.shorter))
Idents(srat) <- "Cell_group.shorter" 
srat <- subset(srat, idents = "Stroma")
...
write.table(data_mat, file = file2write, quote = F, sep = "\t", row.names = T)
