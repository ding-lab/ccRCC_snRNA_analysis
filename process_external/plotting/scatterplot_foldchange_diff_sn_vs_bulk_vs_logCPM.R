...
foldchanges_df <- merge(x = data.frame(gene_symbol = genes_filter),
                        y = young_t_tumorcells_vs_n_nontumor_df %>%
                          ...
                          select(gene_symbol, log2FC.young), by = c("gene_symbol"), all.x = T)
...
foldchanges_df <- merge(x = foldchanges_df,
                        y = wu_t_tumorcells_vs_n_nontumor_df %>%
                          ...
                          select(gene_symbol, log2FC.wu), by = c("gene_symbol"), all.x = T)
...
p <- p + xlab(label = "log2FC (snRNA-seq) - log2FC (bulk RNA-seq)")
p <- p + ylab(label = "log2CPM")
