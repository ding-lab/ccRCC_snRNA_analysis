...
gsea_result_df <- NULL
dir_cnetplot <- paste0(dir_out, "cnetplot/");dir.create(dir_cnetplot)
for (easyid_tmp in unique(deg_df$easy_id)) {
  for (cluster_tmp in unique(deg_df$cluster[deg_df$easy_id == easyid_tmp])) {
    ...
    if (length(enricher_out) > 0 ) {
      ## convert entrez id to gene symbol
      enricher_out <- setReadable(enricher_out, org.Hs.eg.db, keyType = "ENTREZID")
      gsea_result_list[[cluster_name_tmp]] <- enricher_out
      
      enricher_out_df <- enricher_out@result
      gsea_result_df <- rbind(gsea_result_df, enricher_out_df %>%
                                mutate(easy_id = easyid_tmp) %>%
                                mutate(cluster = cluster_tmp))
    }
    enricher_out <- NULL
...
saveRDS(object = gsea_result_list, file = file2write, compress = T)
## store results
file2write <- paste0(dir_out, "GSEA_Results", ".tsv")
write.table(x = gsea_result_df, file = file2write, quote = F, row.names = F, sep = "\t")