...
# p <- p + geom_point(data = subset(plot_data_df), mapping = aes(x = x_plot, y = y_plot, size = size_plot, color = foldchange_type), alpha = 0.5)
...
p <- p + theme_classic()
p <- p + xlab("Bias-corrected accessibility deviations (average)")
p <- p + ylab("-Log10FDR (average)")
