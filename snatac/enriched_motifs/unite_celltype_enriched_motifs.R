...
# input dependencies ------------------------------------------------------
dir_input <- paste0("../ccRCC_snATAC/Resources/snATAC_Processed_Data/Enriched_Motifs")

# get paths to the files of interest -------------------------------------------
paths_process <- list.files(path = dir_input, full.names = T)
paths_process <- paths_process[grepl(x = paths_process, pattern = "MergedObj_motifs.tsv")]
paths_process
...
for (path_process in paths_process) {
  ...
  celltypename <- str_split(string = filename_tmp, pattern = "_chromvar")[[1]][1]
  celltypename
  ## add the cell type
...
# write output ------------------------------------------------------------
file_write <- paste0(dir_out, "Enriched_Motifs.chromvar.MergedObj.byCell_type.", run_id, ".tsv")
