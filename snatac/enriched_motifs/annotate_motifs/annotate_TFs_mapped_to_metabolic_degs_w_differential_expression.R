...
motif_allpeaks_enriched_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snatac/enriched_motifs/plotting/volcano_ccRCC_vs_PT_motifs/20210603.v1/volcano.png")
...
motif2gene_merged_df <- merge(x = motif2gene_filtered_df %>%
                                select(Gene, Peak, Peak_Type, Motif_Type, motif.name), y = motif2tf_de_df, by= c("motif.name"), all.x = T)
