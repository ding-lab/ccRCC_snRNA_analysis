...
avgexp_df1 <- fread(input = "./Resources/snATAC_Processed_Data/Gene_Activity/AverageGeneActivity.ccRCC_markers.Surface.20210928.tsv", data.table = F)
...
plot_data_long_df <- rbind(avgexp_df1, avgexp_df2) %>%
...
# make column split -------------------------------------------------------
column_group_vec <- colanno_df$cell_type
column_group_factor <- factor(x = column_group_vec, levels = c("Proximal tubule", "Tumor cells"))
