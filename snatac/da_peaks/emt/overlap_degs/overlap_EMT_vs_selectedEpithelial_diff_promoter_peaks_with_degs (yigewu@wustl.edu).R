...
peaks2degs_filtered_df <- peaks2degs_df %>%
  ...
  filter((avg_log2FC.snATAC > 0 & avg_log2FC.snRNA > 0) | (avg_log2FC.snATAC < 0 & avg_log2FC.snRNA < 0))
  
# write -------------------------------------------------------------------
file2write <- paste0(dir_out, "EMT_vs_selectedEpithelialClusters.DEG_overlap_DAP.", run_id, ".tsv")
write.table(x = peaks2degs_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "EMT_vs_selectedEpithelialClusters.DEG_overlap_DAP.Consistent.", run_id, ".tsv")
write.table(x = peaks2degs_filtered_df, file = file2write, quote = F, sep = "\t", row.names = F)