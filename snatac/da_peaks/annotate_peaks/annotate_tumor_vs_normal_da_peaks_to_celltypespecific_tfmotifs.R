...
peaks2celltype_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snatac/da_peaks/filter_peaks/filter_celltype4_da_peaks_for_tumor_vs_normal/20200915.v1/DA_peaks.Filter_for_Tumor_vs_Normal.20200915.v1.tsv")
...
motifs2celltype_df <- fread(data.table = F, input = "../ccRCC_snATAC/Resources/snATAC_Processed_Data/4.Cell_type_markers/3.EnrichedMotifs/Tumor_vs_Normal/Proximal tubule_vs_Tumor cells_chromvar_MergedObj_motifs.tsv")
...
peaks2celltype_filtered_df <- peaks2celltype_df %>%
  ...
  mutate(rank_peak_by_avglogFC = order(order(avg_logFC, decreasing = T)))

# filter cell type specific motifs ----------------------------------------
motifs2celltype_filtered_df <- motifs2celltype_df %>%
  filter(p_val_adj < 0.05) %>%
  mutate(pct_diff = (pct.1 - pct.2)) %>%
  mutate(Cell_type.filename = ifelse(avg_logFC < 0, "Tumor cells", "Normal epithelial cells")) %>%
  filter((avg_logFC > 0 & pct_diff > 0.1) | (avg_logFC < 0 & pct_diff < -0.1)) %>%
  group_by(Cell_type.filename) %>%
...
peaks2motif_df <- peaks2motif_df %>%
  filter(V1 %in% peaks2celltype_df$V1)
...
# merge -------------------------------------------------------------------
peaks_merged_df <- merge(peaks2celltype_filtered_df, peaks2motif_filtered_df, by = c("V1"), all.x = T)
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "DA_peaks.Filter_for_Tumor_vs_Normal.MotifAnnotated.", run_id, ".tsv")
