...
degs_snrna_bed1 <- degs_df %>%
  filter(!is.na(Num_sig_up)) %>%
  filter(!is.na(FDR.cnvcorrected) & FDR.cnvcorrected < 0.05) %>%
  filter(avg_lo2FC.alltumorcells < -cutoff_log2FC_snRNA) %>%
  filter(Num_sig_down >=5) %>%
  # filter(Num_up == 0) %>%
...
genes_validatedbybulkRNA_df <- degs_df %>%
  ...
  filter(!is.na(FDR.cnvcorrected) & FDR.cnvcorrected < 0.05) %>%
  filter(!is.na(FDR.bulkRNA) & FDR.bulkRNA < cutoff_fdr_bulkRNA) %>%
  filter((Num_sig_up >=5 & logFC.bulkRNA > cutoff_log2FC_bulkRNA) | (Num_sig_down >=5 & logFC.bulkRNA < -cutoff_log2FC_bulkRNA)) %>%
  filter(!is.na(start_position)) %>%
  mutate(chr = paste0("chr", chromosome_name))
genes_validatedbybulkpro_df <- degs_df %>%
  ...
  filter((Num_sig_up >=5 & meddiff_exp.bulkpro > 0) | (Num_sig_down >=5 & meddiff_exp.bulkpro < 0)) %>%
  filter(!is.na(start_position))
...
highlight_bed <- degs_df %>%
  ...
  mutate(deg_color = ifelse(validated_by_bulkRNA | validated_by_bulkprotein, 
                            ifelse(avg_lo2FC.alltumorcells > 0, "#E41A1C" , "#377EB8"),
                            ifelse(avg_lo2FC.alltumorcells > 0, "#FB9A99" , "#A6CEE3"))) %>%
...
circos.genomicLabels(highlight_bed, labels.column = 4, side = "outside", connection_height = 0.05, labels_height = 0.08, cex = 0.8, col = highlight_bed$deg_color, line_lwd = 0.5, font = highlight_bed$deg_font)
...
circos.genomicTrack(daps_bed_list, 
                    ...
                    }, track.height = 0.125, bg.col = colors_datatypes["snATAC"], bg.border = "grey70")
...
circos.genomicTrack(degs_snrna_bed_list, 
                    panel.fun = function(region, value, ...) {
                      ...
                      color_i <- c("#377EB8", "#E41A1C")[i]
                      circos.genomicPoints(region, value, pch = 16, cex = 0.3, col = color_i, ...)
                    }, track.height = 0.125, bg.col = colors_datatypes["snRNA"], bg.border = "grey70")
circos.yaxis(side = "left", sector.index = "chr1", labels.cex = 0.8, at = c(-1.5, 0, 1.5))
...
circos.genomicTrack(degs_bulkrna_bed_list, 
                    panel.fun = function(region, value, ...) {
                      i = getI(...)
                      color_i <- c("#377EB8", "#E41A1C")[i]
                      circos.genomicPoints(region, value, pch = 16, cex = 0.2, col = color_i, ...)
                    }, track.height = 0.125, bg.col = colors_datatypes["bulkRNA"], bg.border = "grey70")
...
circos.genomicTrack(deps_bed_list,
                    panel.fun = function(region, value, ...) {
                      ...
                      color_i <- c("#377EB8", "#E41A1C")[i]
                      circos.genomicPoints(region, value, pch = 16, cex = 0.3, col = color_i, ...)
                    }, track.height = 0.125, bg.col = colors_datatypes["bulkprotein"], bg.border = "grey70")
