...
peak_info_df <- rbind(peak_info_bap1_df %>%
                      ...
                      peak_info_pbrm1_df %>%
                        mutate(group = "PBRM1"))
peaks_all <- unique(peak_info_df$peak[peak_info_df$p_adjust_bonf < 0.05])
length(peaks_all) ## [1] 11162
peaks_bap1_down <- peak_info_bap1_df$peak[peak_info_bap1_df$p_adjust_bonf < 0.05 & peak_info_bap1_df$avg_log2FC < 0]
peaks_bap1_up <- peak_info_bap1_df$peak[peak_info_bap1_df$p_adjust_bonf < 0.05 & peak_info_bap1_df$avg_log2FC > 0]
peaks_pbrm1_down <- peak_info_pbrm1_df$peak[peak_info_pbrm1_df$p_adjust_bonf < 0.05 & peak_info_pbrm1_df$avg_log2FC < 0]
peaks_pbrm1_up <- peak_info_pbrm1_df$peak[peak_info_pbrm1_df$p_adjust_bonf < 0.05 & peak_info_pbrm1_df$avg_log2FC > 0]
...
plotdata_df <- cbind(plotdata_df, acc_bap1_df[, colnames(acc_bap1_df) %in% peaks_all])
plotdata_df <- cbind(plotdata_df, acc_pbrm1_df[, (colnames(acc_pbrm1_df) %in% peaks_all) & !(colnames(acc_pbrm1_df) %in% colnames(plotdata_df)) ])
...
"#377EB8"
"#E41A1C"
is_bap1_peaks_vec <- ifelse(colnames(plotdata_mat) %in% peaks_bap1_down, "Down-regulated",
                            ifelse(colnames(plotdata_mat) %in% peaks_bap1_up, "Up-regulated", "other"))
is_pbrm1_peaks_vec <- ifelse(colnames(plotdata_mat) %in% peaks_pbrm1_down, "Down-regulated",
                             ifelse(colnames(plotdata_mat) %in% peaks_pbrm1_up, "Up-regulated", "other"))
...
# make column split -------------------------------------------------------
column_split_vec <- ifelse(colnames(plotdata_mat) %in% peaks_pbrm1_down, "PBRM1 down peak",
                           ifelse(colnames(plotdata_mat) %in% peaks_pbrm1_up, "PBRM1 up peak", 
                                  ifelse(colnames(plotdata_mat) %in% peaks_bap1_down, "BAP1 down peak", "BAP1 up peak")))
...
pdf(file2write, width = 13, height=6.5, useDingbats = F)
draw(object = p,
