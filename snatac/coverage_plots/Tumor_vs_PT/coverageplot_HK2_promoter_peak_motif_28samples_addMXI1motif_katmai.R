...
# specify parameters ------------------------------------------------------
motifs_plot <- c("KLF9", "MXI1")
# peak2motif_df %>%
#   filter(Gene == "HK2" & Peak_Type == "Promoter" & motif.name %in% motifs_plot)
...
motifs_plot <- c("MA1107.2", "MA1108.2")
peak_plot <- c("chr2-74833672-74834172", "chr2-74855825-74856325", "chr2-74876177-74876677")
topn_plot <- 24
motif_coord <- c(peak2motif_df$motif_coord[(peak2motif_df$Peak %in% "chr2-74833672-74834172") & (peak2motif_df$group_name %in% "MA1107.2")],
                 peak2motif_df$motif_coord[(peak2motif_df$Peak %in% c("chr2-74855825-74856325", "chr2-74876177-74876677")) & (peak2motif_df$group_name %in% "MA1108.2")])
...
pdf(file2write, width = 10, height = 10, useDingbats = F)
print(p)
dev.off()
