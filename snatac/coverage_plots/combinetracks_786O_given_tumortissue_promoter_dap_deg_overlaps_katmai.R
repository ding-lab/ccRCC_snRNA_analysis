...
peaks_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snatac/da_peaks/bap1/overlap_bap1_associated_peaks_with_degs/20210611.v1/BAP1_DEG_associated_Peaks.20210611.v1.tsv")

# specify peak and gene to plot -------------------------------------------
peaks_plot_df <- peaks_df %>%
  filter(DAP_type == "Promoter") %>%
  mutate(seqnames = str_split_fixed(string = peak, pattern = "\\-", n = 3)[,1]) %>%
  mutate(start = str_split_fixed(string = peak, pattern = "\\-", n = 3)[,2]) %>%
  mutate(end = str_split_fixed(string = peak, pattern = "\\-", n = 3)[,3]) %>%
  mutate(SYMBOL = Gene)
