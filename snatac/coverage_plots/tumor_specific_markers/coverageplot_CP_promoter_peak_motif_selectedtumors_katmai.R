...
head(atac@meta.data)
atac_subset=subset(atac,(cell_type.v20210611 %in% c('Tumor') & Piece_ID %in% pieceids_selected) | cell_type.v20210611=='PT' & Piece_ID %in% c('C3L-00088-N','C3N-01200-N'))
## make colors

color_tumorcell <- RColorBrewer::brewer.pal(n = 9, name = "Dark2")[4]
color_pt <- RColorBrewer::brewer.pal(n = 9, name = "Dark2")[1]
colors_celltype <- c(rep(x = color_tumorcell, length(pieceids_selected)), rep(x = color_pt, 2))
