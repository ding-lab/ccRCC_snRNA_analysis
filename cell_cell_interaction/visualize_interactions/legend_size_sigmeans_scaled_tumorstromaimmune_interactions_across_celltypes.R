...
interactions_filtered_df1 <- fread(data.table = F, input = "./Data Freezes/V1/snRNA/Cell_Cell_Interactions/Values_by_sample_mean_expr_of_curated_interactions_top3ct_stroma_immune_20200818_seen_5+_grouped_by_Inter.group_strongest_in_given_pair.txt")
...
plot_data_wide_df <- dcast(data = plot_data_df, formula = interacting_pair_directed ~ celltypes.source2target, value.var = "avg_sig_mean")
plot_data_mat <- as.matrix(plot_data_wide_df[, -1])
rownames(plot_data_mat) <- plot_data_wide_df$interacting_pair_directed
...
rowanno_obj1 <- rowAnnotation(
  # Gene_source2target = anno_text(x = paste0(gene_source_vec, "->", gene_target_vec), 
...
colanno_obj <- HeatmapAnnotation(Celltype.Ligand = anno_simple(x = vector(mode = "numeric", length = length(col_celltypes.source_vec)), 
                                                               gp = gpar(fill = NA, col = NA), height = unit(10, "mm"), 
                                                               pch = ifelse(col_celltypes.source_vec == "Tumor cells", 23, 21), 
                                                               pt_size = unit(10, "mm"), 
                                 ...
                                 #                         gp = gpar(fill = NA, col = NA), height = unit(8, "mm"), 
                                 #                         pch = 25, pt_size = unit(5, "mm"), pt_gp = gpar(fill = "black")),
                                 ...
                                 Celltype.Receptor = anno_simple(x = vector(mode = "numeric", length = length(col_celltypes.source_vec)),
                                                                 gp = gpar(fill = NA, col = NA), height = unit(10, "mm"), 
                                                                 pch = ifelse(col_celltypes.target_vec == "Tumor cells", 23, 21), 
                                                                 pt_size = unit(10, "mm"), 
...
column_split_factor <- factor(x = column_split_vec, levels = c("Tumor&Stroma", "Tumor&Tumor", "Stroma&Stroma", "Tumor&Immune", "Stroma&Immune"))

# plot hetamp body --------------------------------------------------------
plot_size_df <- dcast(data = plot_data_df, formula = interacting_pair_directed~ celltypes.source2target, value.var = "number_sig_cases")
...
p <- ComplexHeatmap::Heatmap(matrix = plot_data_mat,
                             ...
                             cell_fun = function(j, i, x, y, width, height, fill) {
                               ...
                               if (plot_data_mat[i, j] > 0) {
                                 ...
                                 grid.rect(x = x, y = y, 
                                           ...
                                           gp = gpar(fill = colors_heatmapbody(plot_data_mat[i, j]), col = NA))
                               }},
                             ...
                             ## column parameters
                             column_gap = unit(x = 0.3, units = "cm"),
...
pdf(file2write, width = 22, height = 25)
draw(object = p, 
     annotation_legend_side = "bottom", annotation_legend_list = list_lgd)
dev.off()
