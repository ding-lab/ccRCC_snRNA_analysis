...
# summary_df <- fread(data.table = F, input = "./Resources/Analysis_Results/cell_cell_interaction/summarize_interactions/summarize_across_samples_by_pair/20200923.v1/cellphonedb.summary_across_samples_by_pair.20200923.v1.tsv")
summary_df <- fread(data.table = F, input = "./Resources/Analysis_Results/cell_cell_interaction/annotate_interactions/annotate_cellphone_out/20201012.v1/cell.phone.res.total.run20200818.filtered.formatted.txt")
...
summary_filtered_df <- summary_df %>%
  filter(gene.source == "WNT5A") %>%
  filter(gene.target == "FZD6") %>%
  filter(Cell_type.source != "Mixed myeloid/lymphoid" & Cell_type.target != "Mixed myeloid/lymphoid") %>%
  filter(cell.num.ct1 >= 100 & cell.num.ct2 >= 100)
...
## filter out samples without any significant interactions
plot_data_mat <- plot_data_mat[,colSums(!is.na(plot_data_mat))>0]
...
celltype_source_vec[celltype_source_vec == "Tumor-like epithelial cells"] <- "Transitional cells"
...
celltype_target_vec[celltype_target_vec == "Tumor-like epithelial cells"] <- "Transitional cells"
celltype_text_vec <- paste0(celltype_source_vec, "->", celltype_target_vec)
rowanno_obj1 <- rowAnnotation(CellTypes = anno_text(x = celltype_text_vec, gp = gpar(fontface = "italic", fontsize = 15)),
...
p <- ComplexHeatmap::Heatmap(matrix = plot_data_mat,
                             ...
                             ## column
                             show_column_dend = F, cluster_columns = T, column_names_side = "top",
                             # column_split = sampletypes_vec, 
                             column_title = NULL, 
...
png(file2write, width = 1200, height = 1300, res = 150)
draw(object = p, 
