...
summary_filtered_df <- summary_df %>%
  filter(pair_cell.types %in% c("IGF1_IGF1R.Macrophages|Tumor cells", "EGFR_HBEGF.Tumor cells|Macrophages", "MET_HGF.Tumor cells|Macrophages", "EGFR_TGFA.Tumor cells|Macrophages",
                                "VEGFA_FLT1.Tumor cells|Endothelial cells", "VEGFA_KDR.Tumor cells|Endothelial cells", "FLT1_VEGFB.Endothelial cells|Tumor cells",
                                "ANGPT2_TEK.Endothelial cells|Endothelial cells", "ANGPT2_TEK.Myofibroblasts|Endothelial cells", "TEK_ANGPT1.Endothelial cells|Myofibroblasts")) %>%
  arrange(desc(avg_sig_mean), number_sig_cases)
## extract the ligand genes
genes_filter <- unique(c(summary_filtered_df$gene.source, summary_filtered_df$gene.target))
