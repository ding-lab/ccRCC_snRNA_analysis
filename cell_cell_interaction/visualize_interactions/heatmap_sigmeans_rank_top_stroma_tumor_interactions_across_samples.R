...
colanno_obj <- HeatmapAnnotation(Celltype_source = anno_text(x = paste0("<-", col_celltypes.source_vec), gp = gpar(fill = colors_celltype[col_celltypes.source_vec])),
                                 Celltype_target = anno_text(x = col_celltypes.target_vec, gp = gpar(fill = colors_celltype[col_celltypes.target_vec])))
