...
colors_heatmapbody_legand <- colorRamp2(c(0, 2, 4, 6, 8, 10), 
...
egfr_protein_vec <- as.vector(protein_mat2["EGFR",]); names(egfr_protein_vec) <- ids_aliquot[!is.na(ids_aliquot_bulk)]; egfr_protein_vec <- egfr_protein_vec[ids_aliquot]
igf1_protein_vec <- as.vector(protein_mat2["IGF1",]); names(igf1_protein_vec) <- ids_aliquot[!is.na(ids_aliquot_bulk)]; igf1_protein_vec <- igf1_protein_vec[ids_aliquot]
igf1r_protein_vec <- as.vector(protein_mat2["IGF1R",]); names(igf1r_protein_vec) <- ids_aliquot[!is.na(ids_aliquot_bulk)]; igf1r_protein_vec <- igf1r_protein_vec[ids_aliquot]
hgf_protein_vec <- as.vector(protein_mat2["HGF",]); names(hgf_protein_vec) <- ids_aliquot[!is.na(ids_aliquot_bulk)]; hgf_protein_vec <- hgf_protein_vec[ids_aliquot]
met_protein_vec <- as.vector(protein_mat2["MET",]); names(met_protein_vec) <- ids_aliquot[!is.na(ids_aliquot_bulk)]; met_protein_vec <- met_protein_vec[ids_aliquot]

colanno_obj1 <- HeatmapAnnotation(EGFR.bulk.protein = anno_simple(x = egfr_protein_vec, col = colors_protein),
                                  IGF1.bulk.protein = anno_simple(x = igf1_protein_vec, col = colors_protein),
                                  IGF1R.bulk.protein = anno_simple(x = igf1r_protein_vec, col = colors_protein),
                                  HGF.bulk.protein = anno_simple(x = hgf_protein_vec, col = colors_protein),
                                  MET.bulk.protein = anno_simple(x = met_protein_vec, col = colors_protein),
