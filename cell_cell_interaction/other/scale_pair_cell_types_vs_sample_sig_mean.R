...
cellphone_scaled_mat1 <- scale(x = cellphone_wide_mat, center = T, scale = T)
cellphone_scaled_mat1[1:5, 1:10]
cellphone_scaled_mat2 <- t(scale(x = t(cellphone_scaled_mat1), center = T, scale = T))
cellphone_scaled_mat2[1:5, 1:10]
...
write.table(x = cellphone_scaled_mat1, file = file2write, quote = F, sep = "\t", row.names = T)
file2write <- paste0(dir_out, "cellphonedb.pair_cell.types_vs_sample.sig_mean.scaled_by_sample_bypair.", ".tsv")
write.table(x = cellphone_scaled_mat2, file = file2write, quote = F, sep = "\t", row.names = T)
