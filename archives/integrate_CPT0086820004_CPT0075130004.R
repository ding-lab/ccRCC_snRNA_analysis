...
## v2, used correct seurat object
## v3, remove all the individual downstream result of findvariablefeature in the object (doesn't matter in the end)
...
# Log-normalization & identify variable features individually -------------
...
for (i in 1:length(renal.list)) {
  renal.list[[i]] <- NormalizeData(renal.list[[i]])
  renal.list[[i]] <- FindVariableFeatures(renal.list[[i]], selection.method = "vst", 
                                          nfeatures = 3000) 
}
...
renal.anchors <- FindIntegrationAnchors(object.list = reference.list, dims = 1:30)
...
renal.integrated <- IntegrateData(anchorset = renal.anchors, dims = 1:30)
...
renal.integrated <- SCTransform(renal.integrated, vars.to.regress = c("nCount_RNA","percent.mito")) 
renal.integrated <- RunPCA(renal.integrated)
renal.integrated <- FindNeighbors(renal.integrated, dims = 1:30)
renal.integrated <- FindClusters(renal.integrated)
renal.integrated <- RunUMAP(renal.integrated, dims= 1:30)
...
renal.markers %>% group_by(cluster) %>% top_n(2, avg_logFC)
...
FeaturePlot(object = object2plot, features = top.markers, cols = c("grey", "blue"), reduction = "umap")
dev.off()
...
## plot RCC markers
pdf(paste(marker_exp_out_path,"RCC_marker_expression_in_sample_", sample_id, ".pdf", sep=""),height=16,width=12)
top.markers <- c("NPHS2", "LRP2", "AQP1", "AQP2", "UMOD")
FeaturePlot(object = object2plot, features = top.markers, cols = c("grey", "blue"), reduction = "umap")
dev.off()

## plot RCC oncogenes
pdf(paste(marker_exp_out_path,"RCC_Oncogenes_expression_in_sample_", sample_id, ".pdf", sep=""),height=16,width=14)
top.markers <- c("HIF1A", "EPAS1" , "MTOR", "MET", "MAP2K1", "MAP2K2", "MAPK1", "MAPK3")
FeaturePlot(object = object2plot, features = top.markers, cols = c("grey", "blue"), reduction = "umap")
dev.off()

## plot RCC TSGs
pdf(paste(marker_exp_out_path,"RCC_TSG_expression_in_sample_", sample_id, ".pdf", sep=""),height=16,width=12)
top.markers <- c("VHL", "PBRM1" , "BAP1", "SETD2", "PTEN", "TP53")
FeaturePlot(object = object2plot, features = top.markers, cols = c("grey", "blue"), reduction = "umap")
dev.off()

## plot Kidney Proximal Tubule Markers
pdf(paste(marker_exp_out_path,"Proximal_Tubule_Marker_expression_in_sample_", sample_id, ".pdf", sep=""), height=14,width=12)
