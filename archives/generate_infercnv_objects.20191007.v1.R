...
dir_infercnv_out <- "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/outputs/"
dir.create(dir_infercnv_out)
dir_int_obj <- "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/integration/"
...
tab_gene_order_symbol = read.delim(file = "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/inputs/gencode_v21_gen_pos.MatchCellRangerFeatures.NoDuplicates.20191005.v1.txt", header = FALSE,stringsAsFactors = FALSE)
...
rm(raw_exp_mat)
# write.table(x = clean_exp_mat, file = paste0(dir_infercnv_out, "RNA_Count.", sample_id, ".", run_id, ".tsv"), quote = F, row.names = T, col.names = T, sep = "\t")
...
# run infercnv ------------------------------------------------------------
sink(file = paste0(dir_infercnv_out_by_int_sample, sample_id, ".Log.", run_id, ".txt"))
infercnv_obj = infercnv::CreateInfercnvObject(raw_counts_matrix=clean_exp_mat,
                                    annotations_file=paste0(dir_infercnv_out_by_int_sample, sample_id, ".Barcode_Annotation.", run_id, ".txt"),
                                    delim="\t",
                                    gene_order_file= paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/inputs/gencode_v21_gen_pos.MatchCellRangerFeatures.NoDuplicates.20191005.v1.txt"),
                                    ref_group_names= ref_group_names)
saveRDS(object = infercnv_obj, file = paste0(dir_infercnv_out_by_int_sample, sample_id, ".infercnv_obj.", run_id, ".RDS"))
sink()
