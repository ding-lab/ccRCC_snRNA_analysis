...
# check the plots grouped by sample and grouped by cluster -------------
...
outpath <- makeOutDir()
marker_exp_out_path <- paste0(outpath, "RCC_Marker_Expression/")
dir.create(marker_exp_out_path)

plot_parameters <- list()

plot_parameters[["RCC"]] <- list()
plot_parameters[["RCC"]][["top.markers"]] <- c("NPHS2", "LRP2", "AQP1", "AQP2", "UMOD")
plot_parameters[["RCC"]][["FeaturePlot"]] <- list() 
plot_parameters[["RCC"]][["FeaturePlot"]][["width"]] <- 12
plot_parameters[["RCC"]][["FeaturePlot"]][["height"]] <- 16
plot_parameters[["RCC"]][["VlnPlot"]] <- list() 
plot_parameters[["RCC"]][["VlnPlot"]][["width"]] <- 6
plot_parameters[["RCC"]][["VlnPlot"]][["height"]] <- 16
...
gene2cellType_tab <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/Kidney_Markers/RCC_literature_review_and_marker_gene_table - Gene_to_Cell_Type_Table_20190909_v1.tsv")
for (cell_type_tmp in unique(gene2cellType_tab$Marker_of)) {
  marker_genes_tmp <- gene2cellType_tab$Gene[gene2cellType_tab$Marker_of == cell_type_tmp]
  renal.markers[, cell_type_tmp] <- as.numeric(renal.markers$gene %in% marker_genes_tmp)
}
saveRDS(object = renal.markers, file = paste0(makeOutDir(), "renal.markers.pos.RDS"))

renal.markers.filtered <- renal.markers[rowSums(renal.markers[,unique(gene2cellType_tab$Marker_of)]) > 0,]
...
## Confident cluster 0: CA9+ ccRCC #1
### Defining markers:
#### ccRCC: CA9, LRP2
...
## cluster 1: Fetal-like (PAX2 CapMesenchyme, TFCP2L1 UretericBud)

## cluster 2: Distal Tubule (SLC8A1) & Glomerulus (PTPRO) & Vascular (PECAM1)

## cluster 3: Endothelial cell (VCAM1)

## cluster 4: Proximal_convoluted

## cluster 5: Distal Tubuele (SLC8A1) & Vascular (PECAM1)

## cluster 6: Proximal (SLC13A3) & MET & Ureteric Bud (ELF3)

## cluster 7: Endothelial (Vascular marker VCAM1)

## cluster 8: Myofibroblast
### Defining markers:
#### Myofibroblast: PDGFRB, ACTA2
### Confounding Markers
#### Vascular: PLVAP, PTPRB
#### ccRCC: NDIFA4L2

## cluster 9: Endothelial (Endothelial marker VWF, FLT1, PTPRB, KDR, PLVAP, PECAM1; Glomerulus marker PODXL )

## cluster 10: (Distal marker SLC8A1; Glomerulus marker PTPRO; Vascular marker PECAM1)

## Confident cluster 11: CA9+ ccRCC #2
### Defining markers:
#### ccRCC: CA9, LRP2
#### Proximal convoluted: SLC17A3

## cluster 12: (Distal marker SLC8A1; Glomerulus marker PTPRO)

## cluster 13: (Distal marker SLC8A1; Bascular marker PECAM1)
