...
write.table(immune.markers, file = paste0(makeOutDir(), "Immune.DEGs.Pos.txt"), quote = F, sep = "\t", row.names = F)
...
write.table(immune.markers, file = paste0(makeOutDir(), "Immune.markers.Pos.CellTypeMarkerAnnotated",  ".", format(Sys.Date(), "%Y%m%d") , ".v", version_tmp, ".txt"), quote = F, sep = "\t", row.names = F)
...
write.table(immune.markers.filtered, file = paste0(makeOutDir(), "Immune.markers.Pos.CellTypeMarkerOnly", ".", format(Sys.Date(), "%Y%m%d") , ".v", version_tmp, ".txt"), quote = F, sep = "\t", row.names = F)
