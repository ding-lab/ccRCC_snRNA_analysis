...
features_pca <- srat@assays$SCT@var.features
features_pca <- c(features_pca, gene2celltype_df$Gene) %>% unique()
length(features_pca)
srat <- RunPCA(srat, npcs = num_pc, verbose = FALSE, features = features_pca)
cat("###########################################\n")
