...
pathway_genes <- as.vector(ccrcc_gene_alt_downstream_genes_df$target_genesymbol[ccrcc_gene_alt_downstream_genes_df$source_genesymbol %in% c("HIF1A", "EPAS1")])
...
avg_pathway_scaled_avgexp_df <- data.frame(exp_manual_subcluster_name = str_split_fixed(string = names(avg_pathway_scaled_avgexp_mat), pattern = "\\.", n = 2)[,2],
                                           ...
                                           pathway_name = "HIF")
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "hif_pathway.", "avg_pathway_scaled_avgexp.", run_id, ".tsv")
