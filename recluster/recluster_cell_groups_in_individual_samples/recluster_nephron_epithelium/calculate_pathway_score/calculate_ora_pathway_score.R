...
ora_pathway_df <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/integration/30_aliquot_integration/findvariablefeatures/findvariablefeatures_tumor_cells/20200310.v1/ORA_results_Pathway.tsv", data.table = F)
...
for (path_tmp in ora_pathway_df$pathway) {
  # get the list of eligible HIF pathway members ----------------------------
  pathway_genes_string <- ora_pathway_df$members_input_overlap[ora_pathway_df$pathway == path_tmp]
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "ora_pathway.", "avg_pathway_scaled_avgexp.", run_id, ".tsv")
write.table(x = avg_pathway_scaled_avgexp_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "ora_pathway.", "pathway_members.", run_id, ".RDS")
