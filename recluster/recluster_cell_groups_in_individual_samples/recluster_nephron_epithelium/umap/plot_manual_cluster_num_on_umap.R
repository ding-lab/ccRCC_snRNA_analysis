...
for (aliquot_tmp in srat_paths$Aliquot) {
  ...
  stop("")
  metadata_tmp_df <- srat@meta.data
  metadata_tmp_df$barcode <- rownames(metadata_tmp_df)
  metadata_tmp_df <- merge(metadata_tmp_df, barcode2manualsubcluster_df, by = c("orig.ident", "barcode"), all.x = T)
  metadata_tmp_df$manual_cluster_id[is.na(metadata_tmp_df$manual_cluster_id)] <- "NA"
  rownames(metadata_tmp_df) <- metadata_tmp_df$barcode
  srat@meta.data <- metadata_tmp_df
  Idents(srat) <- "manual_cluster_id"
  
  ## make color for each cluster
  uniq_cluster_colors <- Polychrome::dark.colors(n = length(unique(srat@meta.data$manual_cluster_id)))
  names(uniq_cluster_colors) <- unique(srat@meta.data$manual_cluster_id)
  
  ## make plot
  p <- DimPlot(object = srat)
  p <- p + ggtitle(label = paste0(aliquot_tmp, " Tumor-Cell-Only Clustering"), subtitle = "Manual Grouping")
