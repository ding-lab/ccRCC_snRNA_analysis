...
cluster2celltype_df <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/snRNA_Processed_Data/Cell_Type_Assignment/ccRCC_snRNA_Downstream_Processing - Individual.AllCluster2Cell_Type.20200220.v1.tsv", data.table = F)
...
for (aliquot_tmp in seurat_summary2process$Aliquot) {
  ...
  file2write <- paste0(dir_out1, aliquot_tmp, ".immune_reclustered.", run_id, ".RDS")
  if (!file.exists(file2write)) {
    ...
    clusters2process <- cluster2celltype_df$Cluster[cluster2celltype_df$Aliquot == aliquot_tmp & cluster2celltype_df$Most_Enriched_Cell_Group == "Immune"]
    clusters2process
