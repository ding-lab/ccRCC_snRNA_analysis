...
for (snRNA_aliquot_id_tmp in seurat_summary2process$Aliquot) {
  ...
  file2write <- paste0(dir_out1, snRNA_aliquot_id_tmp, ".immune_reclustered.", run_id, ".RDS")
  if (!file.exists(file2write)) {
    ...
    clusters2process <- cluster2celltype_df$Cluster[cluster2celltype_df$Aliquot == snRNA_aliquot_id_tmp & cluster2celltype_df$Most_Enriched_Cell_Group == "Immune"]
    clusters2process
