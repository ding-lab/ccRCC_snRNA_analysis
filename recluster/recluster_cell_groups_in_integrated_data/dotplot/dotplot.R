...
## argument 4: path to the cell type marker table
path_gene2celltype_df <- args[4]
cat(paste0("Path to the cell type marker table: ", path_gene2celltype_df, "\n"))
cat("###########################################\n")

## argument 5: minimal percentage of cell in any clustering expressing the genes to show
min.exp.pct <- args[5]
...
## save dotplot
png(file = path_output, width = 4500, height = 1800, res = 150)
print(p)
dev.off()
