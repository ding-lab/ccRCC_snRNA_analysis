...
# filter
panc <- FilterCells(object = panc, subset.names = c("nGene", "nUMI", "percent.mito"), low.thresholds = c(200, 1000, -Inf), high.thresholds = c(6000, 50000, 0.10))
nrow(FetchData(panc))
# TWAN-RCC-D2: 841
...
# cluster0 marker gene:IL7R, CCD7(T cells- consistts of cd4+ and NK)
# cluster1 marker gene:ITGAX (DC - kind of)
# cluster2 marker gene:CRYAB(Epithelial + bit tumor from LRP2)
# cluster3 marker gene:CD163, APOC1 (Macrophage)
# cluster4 marker gene:CD163, APOC1 (Epithelial + no tumor)
# cluster5 marker gene:CD8A (CD8+ T-cells)
# cluster6 marker gene:COL1A1(Astrocytes/Mesangial cells)
...
new.cluster.ids <- c("T-cells (CD4+, NK)", "DC (kind of)", "Epithelial(rc, stem)", "Macrophage", "Epithelial(rc)", "CD8+ T-cells", "Astrocytes/Mesangial" )
...
TSNEPlot(object = panc, do.label = TRUE, pt.size = 1, label.size = 6, no.legend = TRUE, colors.use=brewer.pal(7, "Set1"))
dev.off()
...
ggplot(tmp, aes(x="", y=Freq, fill=Var1)) +
  ...
  scale_fill_manual(values=brewer.pal(7, "Set1"), name=paste("Cell type", sum(tmp$Freq), sep=": "), labels=myLegend) +
  blank_theme +
