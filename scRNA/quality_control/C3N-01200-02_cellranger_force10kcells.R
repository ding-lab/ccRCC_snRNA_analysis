...
# Set IDs -----------------------------------------------------------------
sample_id <- "C3N-01200-02_force10kcells"
...
# Filtering cells with expression sum per cell ----------------------------
## otherwise if it's too big cannot get Seurat object
## Aiming for 10k cells

## filter by expression sum and get the cell barcodes with expression sum over 1000 under 50000 (#cells: 78572)
bg_id <- names(bc_sums[bc_sums >= 1000 & bc_sums <= 50000])
bg_id %>% length()

## filter by expression sum and get the cell barcodes with expression sum over 1500 under 50000 (#cells: 43734)
bg_id <- names(bc_sums[bc_sums >= 1500 & bc_sums <= 50000])
bg_id %>% length()

## filter by expression sum and get the cell barcodes with expression sum over 2000 under 50000 (#cells: 23028)
bg_id <- names(bc_sums[bc_sums >= 2000 & bc_sums <= 50000])
bg_id %>% length()

## filter by expression sum and get the cell barcodes with expression sum over 2500 under 50000 (#cells: 18108)
bg_id <- names(bc_sums[bc_sums >= 2500 & bc_sums <= 50000])
bg_id %>% length()

## filter by expression sum and get the cell barcodes with expression sum over 3000 under 50000 (#cells: 14539)
bg_id <- names(bc_sums[bc_sums >= 3000 & bc_sums <= 50000])
bg_id %>% length()

## filter by expression sum and get the cell barcodes with expression sum over 3500 under 50000 (#cells: 11778)
bg_id <- names(bc_sums[bc_sums >= 3500 & bc_sums <= 50000])
bg_id %>% length()

input = input[,bg_id]
saveRDS(object = input, file = paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/scRNA/", sample_id, "/", sample_id, "_matrix_ccounts_over3500_under50000_per_cell.RDS"))

rm(bc_sums)
rm(bg_id)

# if run after the counts are filtered, input the RDS object for the matrix --------------------------------------------------
...
file2write <- paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/scRNA/", sample_id, "/", sample_id, "_primary_seurat_object_counts_over3500_under50000_per_cell.RDS")
saveRDS(object = primary_object, file = file2write)

# plot Pre-filtering QC metrics ---------------------------------------------------------
file2write <- paste(makeOutDir(), "Pre-Filtering_QC_metrics_in_sample_", sample_id, ".pdf", sep="")
...
# get cutoffs
info <- as.data.frame(x=FetchData(primary_object, vars=c("nCount_RNA","nFeature_RNA", "percent.mito")))

m <- vector()
n <- vector()
for(i in seq(300, 3000, 100)) {
  tmp <- info[info$nFeature_RNA > i, ]
...
file2write <- paste(makeOutDir(), "Pre-Filtering_Different_UMI_cutoff_in_sample_", sample_id, ".pdf", sep="")
pdf(file = file2write, width=15, height=9)
par(mar = c(5, 5, 3, 5))
...
qced_object <- subset(primary_object, subset = nCount_RNA > 3500 &  nCount_RNA < 50000)
qced_object@meta.data %>%
  dim()
# [1] 11774     4

qced_object <- subset(primary_object, subset = nCount_RNA > 3500 &  nCount_RNA < 50000 & percent.mito < 0.10)
qced_object@meta.data %>%
  dim()
# [1] 9559    4

qced_object <- subset(primary_object, subset = nFeature_RNA > 200 & nFeature_RNA < 5000 & nCount_RNA > 3500 &  nCount_RNA < 50000 & percent.mito < 0.10)
qced_object@meta.data %>%
  dim()
# [1] 8568    4

qced_object <- subset(primary_object, subset = nFeature_RNA > 200 & nFeature_RNA < 5000 & nCount_RNA > 4000 &  nCount_RNA < 50000 & percent.mito < 0.10)
qced_object@meta.data %>%
  dim()
# [1] 8568    4
...
file2write <- paste(makeOutDir(), "Post-Filtering_QC_metrics_in_sample_", sample_id, ".20190816_v1.pdf", sep="")
pdf(file = file2write, width=15, height=9)
...
## write out the expression matrics for polluck
...
# file2write <- paste(makeOutDir(), "DimPlot_pollock_cell_type_in_sample_",sample_id, ".20190816_v2.pdf", sep="")
# pdf(file = file2write, width=12, height=12)
