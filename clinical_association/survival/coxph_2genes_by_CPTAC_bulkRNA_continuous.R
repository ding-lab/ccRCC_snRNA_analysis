...
result_list<-foreach(i=1:nrow(combo_index_mat)) %dopar% {
  gene1_tmp <- genes_process[combo_index_mat[i,1]]
  gene2_tmp <- genes_process[combo_index_mat[i,2]]
  
  genes_process_tmp <- c(gene1_tmp, gene2_tmp)
  ...
  testdata_df <- testdata_df %>%
    ...
    dplyr::mutate(EFS_censor = (with_new_event == "Tumor Free")) %>%
  ...
  fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ Expression.gene1 + Expression.gene2, data = testdata_comp_df)
  fit_efs_sum <- summary(fit_efs)
  test_vec <- c(genes_process_tmp,
                ...
                fit_efs_sum$coefficients[1,1], fit_efs_sum$coefficients[2,1])
  return(test_vec)
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Cox_2genes.bulkRNA.continuous.", run_id, ".tsv")
write.table(x = cox_result_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "Cox_2genes.bulkRNA.continuous.Filtered.", run_id, ".tsv")
