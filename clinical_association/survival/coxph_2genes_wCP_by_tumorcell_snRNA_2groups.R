...
for (gene2_tmp in gene2_process) {
  ...
  testdata_df <- testdata_df %>%
    ...
    mutate(Expression_group.gene1 = ifelse(Expression.gene1 < quantile(x = testdata_df$Expression.gene1, probs = 0.5, na.rm = T), "Low", "High")) %>%
    mutate(Expression_group.gene2 = ifelse(Expression.gene2 < quantile(x = testdata_df$Expression.gene2, probs = 0.5, na.rm = T), "Low", "High")) %>%
  ...
  testdata_comp_df$Expression_group.gene1 <- factor(x = testdata_comp_df$Expression_group.gene1, levels = c("High", "Low"))
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Cox_2genes.snRNA.2groups.", run_id, ".tsv")
