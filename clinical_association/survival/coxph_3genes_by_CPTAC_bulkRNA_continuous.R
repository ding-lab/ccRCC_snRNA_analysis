...
gene12_process_df <- fread(data.table = F, input = "./Resources/Analysis_Results/clinical_association/survival/coxph_2genes_by_CPTAC_bulkRNA_continuous/20210920.v2/Cox_2genes.bulkRNA.continuous.Filtered.20210920.v2.tsv")

result_list <- list()
# for (gene_test in "CP") {
for (i in 1:nrow(gene12_process_df)) {
  result_list[[i]] <- list()
  gene1_tmp <- gene12_process_df$genesymbol1[i]
  gene2_tmp <- gene12_process_df$genesymbol2[i]
  gene3_process <- genes_process[!(genes_process %in% c(gene1_tmp, gene2_tmp))]
  for (gene3_tmp in gene3_process) {
    genes_process_tmp <- c(gene1_tmp, gene2_tmp, gene3_tmp)
    ...
    fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ Expression.gene1 + Expression.gene2 + Expression.gene3, data = testdata_comp_df)
    fit_efs_sum <- summary(fit_efs)
    result_list[[i]][[gene3_tmp]] <- c(genes_process_tmp,
                  ...
                  fit_efs_sum$coefficients[1,1], fit_efs_sum$coefficients[2,1], fit_efs_sum$coefficients[3,1])
  }
}
...
colnames(cox_result_df) <- c("genesymbol1", "genesymbol2", "genesymbol3", "pvalue.wald", "pvalue.lr", "pvalue.logrank", 
                             "concordance", "se.concordance",
                             "coef.gene1", "coef.gene2", "coef.gene3")
...
cox_result_df$fdr.logrank <- p.adjust(cox_result_df$pvalue.logrank, method = "fdr")
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Cox_3genes.bulkRNA.continuous.", run_id, ".tsv")
write.table(x = cox_result_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "Cox_3genes.bulkRNA.continuous.Filtered.", run_id, ".tsv")
