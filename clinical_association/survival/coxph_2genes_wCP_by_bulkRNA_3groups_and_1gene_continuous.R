...
gene3_process <- c("PCSK6", "ABCC3", "NDRG1", "COL23A1")

result_list <- list()
# for (gene_test in "CP") {
for (gene2_tmp in gene2_process) {
  result_list[[gene2_tmp]] <- list()
  for (gene3_tmp in gene3_process) {
    genes_process_tmp <- c("CP", gene2_tmp, gene3_tmp)
    exp_test_wide_df <- exp_data_df[exp_df$V1 %in% genes_process_tmp,]
    testdata_df <- data.frame(t(exp_test_wide_df))
    genes_process_colnames <- exp_df$V1[exp_df$V1 %in% genes_process_tmp]
    colnames(testdata_df)[genes_process_colnames == "CP"] <- "Expression.gene1"
    ...
    fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ Expression_group.gene1 + Expression_group.gene2 + Expression.gene3, data = testdata_comp_df)
    fit_efs_sum <- summary(fit_efs)
    result_list[[gene2_tmp]][[gene3_tmp]] <- c(genes_process_tmp,
...
colnames(cox_result_df) <- c("genesymbol1", "genesymbol2", "genesymbol3", "pvalue.wald", "pvalue.lr", "pvalue.logrank", 
                             ...
                             "coef.gene1.medium", "coef.gene1.low", "coef.gene2.low", "coef.gene2.medium", "coef.gene3")
