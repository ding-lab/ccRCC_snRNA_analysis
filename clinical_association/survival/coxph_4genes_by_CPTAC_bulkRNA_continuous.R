...
gene123_process_df <- fread(data.table = F, input = "./Resources/Analysis_Results/clinical_association/survival/coxph_3genes_by_CPTAC_bulkRNA_continuous/20210920.v2/Cox_3genes.bulkRNA.continuous.Filtered.20210920.v2.tsv")
...
for (i in 1:nrow(gene123_process_df)) {
  ...
  for (gene4_tmp in gene4_process) {
    ...
    fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ Expression.gene1 + Expression.gene2 + Expression.gene3 + Expression.gene4, data = testdata_comp_df)
