...
genes_process <- c("CP", "PCSK6")
exp_test_wide_df <- exp_data_df[exp_df$V1 %in% genes_process,]
testdata_df <- data.frame(t(exp_test_wide_df))
colnames(testdata_df) <- exp_df$V1[exp_df$V1 %in% genes_process]
...
fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ CP + PCSK6, data = testdata_comp_df)
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Cox_univariate_continuous_variable.", run_id, ".tsv")
