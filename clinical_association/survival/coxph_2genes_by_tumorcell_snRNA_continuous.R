...
# cox_result_df <- fread(data.table = F, input = "./Resources/Analysis_Results/clinical_association/survival/coxph_univariate_specified_gene_by_tumorcell_snRNA_continuous/20210916.v1/Cox_univariate_continuous_variable.20210916.v1.tsv")
# genes_process <- cox_result_df$gene_symbol[cox_result_df$coef > 0]
genes_process <- c("PCSK6", "ABCC3", "NDRG1", "COL23A1")
combo_index_mat <- comboSample(length(genes_process), 2, n = choose(n = length(genes_process), k = 2))

# test --------------------------------------------------------------------
test_list<-foreach(i=1:nrow(combo_index_mat)) %dopar% {
  genes_process_tmp <- genes_process[combo_index_mat[i,]]
  exp_test_wide_df <- exp_data_df[exp_df$V1 %in% genes_process_tmp,]
  testdata_df <- data.frame(t(exp_test_wide_df))
  colnames(testdata_df) <- c("gene1", "gene2")
  ...
  fit_efs <- coxph(formula = Surv(EFS, EFS_censor == 0) ~ gene1 + gene2, data = testdata_comp_df)
  fit_efs_sum <- summary(fit_efs)
  test_vec <- c(exp_df$V1[exp_df$V1 %in% genes_process_tmp], 
                ...
                fit_efs_sum$coefficients[1, "coef"], fit_efs_sum$coefficients[2, "coef"],
                fit_efs_sum$coefficients[1, 5], fit_efs_sum$coefficients[2, 5])
  return(test_vec)
}
test_result_df <- do.call(rbind.data.frame, test_list)
colnames(test_result_df) <- c("gene1", "gene2", "pvalue.wald", "pvalue.lr", "pvalue.logrank", "concordance", "se.concordance", "coef1", "coef2", "pvalue1", "pvalue2")
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Cox_2genes.continuous_variable.", run_id, ".tsv")
