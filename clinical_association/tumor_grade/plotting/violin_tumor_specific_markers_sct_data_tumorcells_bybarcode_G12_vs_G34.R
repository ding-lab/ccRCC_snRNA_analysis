...
path_log <- paste0(dir_out, "G12_vs_G34_T_test.txt")
sink(path_log)
...
# for (gene_tmp in colnames(exp_df)) {
  ...
  p = p + stat_compare_means(data = plot_data_df,
                             mapping = aes(x = x_plot, y = y_plot, label = ..p.signif..),
  ...
  fc_34vs12 <- median(plot_data_df$expression[plot_data_df$Histologic_Grade %in% c("G3", "G4")])/exp_med12
  ...
  cat(paste0(gene_tmp, ":\nG3&4 vs. G1&2: t = ", signif(x = t.test_result_34vs12$statistic, digits = 2), ", p-value = ", signif(x = t.test_result_34vs12$p.value, digits = 2), "\n\n\n"))
  cat(paste0(gene_tmp, ":\nG3&4 vs. G1&2: ", signif(x = fc_34vs12, digits = 2), "\n\n\n"))
}
sink()
