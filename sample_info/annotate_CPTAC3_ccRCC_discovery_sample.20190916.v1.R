...
cptac_shiped2wustl_sample_map <- read_excel("Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/sample_info/02_Post_request_CPTAC3_shipment/JHU Distribution Manifest_ccRCC material to WUSTL_6.17.2019.xlsx")
...
genes2add <- c("VHL", "PBRM1", "BAP1", "SETD2", "MTOR")
maf_tab <- loadMaf()
...
# add MTOR:EIF4EBP1:S65 outlier -----------------------------------------------
partIDs <- case_annotation$partID
partIDs_tmp <- intersect(partIDs, colnames(mut_mat))
mtor_mut_partIDs <- partIDs_tmp[which(mut_mat[mut_mat$Hugo_Symbol == "MTOR",partIDs_tmp] != "")]
mtor_mut_partIDs

file2input <- paste0("./Ding_Lab/Projects_Current/PanCan_Phospho-signaling/analysis_results/phospho_network/es_score/table/calculate_es_pair_score/kinase/sscore_tab_", "CCRCC", "_", "kinase", "_reg_nonNA", "20", ".txt")
sscore_tab <- fread(input = file2input, data.table = F)
sscore_tab_scaled <- sscore_tab[, partIDs_tmp]
sscore_tab_scaled <- scale_by_row(sscore_tab_scaled)
sscore_tab_outlier <- cbind(sscore_tab[, c("SUB_GENE", "SUB_MOD_RSD")], (sscore_tab_scaled > outlier_sd))

pho_outlier_tmp <- sscore_tab_outlier[sscore_tab_outlier$SUB_MOD_RSD == "S65" & sscore_tab_outlier$SUB_GENE == "EIF4EBP1",]
mtor_pho_outlier_tmp <- mtor_mut_partIDs[which(pho_outlier_tmp[1, mtor_mut_partIDs] == T)]

# add EGFR phosphosite outlier -----------------------------------------------
partIDs <- case_annotation$partID
partIDs_tmp <- intersect(partIDs, colnames(mut_mat))
mtor_mut_partIDs <- partIDs_tmp[which(mut_mat[mut_mat$Hugo_Symbol == "MTOR",partIDs_tmp] != "")]
mtor_mut_partIDs
