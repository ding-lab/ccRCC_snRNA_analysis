...
# merge -------------------------------------------------------------------
clinical_df <- merge(idmetadata_df, clinical_sup_df, by.x = c("Case", "Sample"), by.y = c("case_id", "specimen_id"), all.x = T)
...
clinical_selected_df <- clinical_df %>%
  ...
  mutate(Histologic_Type = ifelse(Sample_Type == "Tumor", Histologic_Type, "Normal Adjacent Tissue")) %>%
  select(Case, Sample, Sample_Type, Aliquot.snRNA, Aliquot.bulk, Aliquot.snRNA.WU, snRNA_available, snATAC_available,
...
clinical_selected_df$Histologic_Grade[clinical_selected_df$Aliquot.snRNA.WU == "C3N-01200-T1"] <- "G2"
clinical_selected_df$Histologic_Grade[clinical_selected_df$Aliquot.snRNA.WU == "C3N-01200-T2"] <- "G4"
clinical_selected_df$Histologic_Grade[clinical_selected_df$Aliquot.snRNA.WU == "C3N-01200-T3"] <- "G4"
## correct normal samples
normal_sample_ids <- idmetadata_df$Sample[idmetadata_df$Sample_Type == "Normal"]
clinical_selected_df$Histologic_Grade[clinical_selected_df$Sample %in% normal_sample_ids] <- "NAT"
clinical_selected_df$Histologic_Grade
clinical_selected_df$Histologic_Type

# write output ------------------------------------------------------------
write.table(x = clinical_selected_df, file = paste0(dir_out, "snRNA_ccRCC_Specimen_Clinicl_Data.", run_id, ".tsv"), quote = F, sep = "\t", row.names = F)
