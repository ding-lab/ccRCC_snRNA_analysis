...
## 2021-03-05 add 3 more snATAC samples
...
snrna_sample_df <- fread(data.table = F, input = "./Resources/snRNA_Processed_Data/scRNA_auto/summary/Seurat_Preprocessing.20210729.tsv")
## input samples with snATAC data
snatac_sample_df <- fread(data.table = F, input = "./Resources/snATAC_Processed_Data/Selection_Rationale/snATAC_Sample_Selection_Rationale.20210809.tsv")
...
id_metadata_df <- id_metadata_df %>%
  ...
  mutate(snATAC_used = ifelse(Aliquot.snRNA.WU %in% c("C3N-00177-T1"), FALSE, snATAC_available))
...
# sort --------------------------------------------------------------------
id_metadata_df <- id_metadata_df %>%
  arrange(-snRNA_available, -snATAC_available, -FFPE_available_slides)
