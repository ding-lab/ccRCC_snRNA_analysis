...
gene2cellType_tab <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/Kidney_Markers/RCC_marker_gene_table_and_literature_review - Gene_to_Cell_Type_Table.20190923.v2.tsv")
...
marker_tab <- merge(marker_tab, gene2cellType_tab %>%
                      select(Gene, Cell_Type_Abbr, PMIDs, Comments), by = c("Gene"), all.x = T)
marker_tab <- merge(marker_tab, proteinatlas_tab, by = c("Gene"), all.x = T)
write.table(x = marker_tab, file = paste0(makeOutDir(), "RCC_Gene2CellType_Tab_w.HumanProteinAtlas", ".", format(Sys.Date(), "%Y%m%d") , ".v", version_tmp ,".tsv"), quote = F, sep = "\t", row.names = F)
