...
srat@meta.data$id_bycluster_bycellgroup_byaliquot <- mapvalues(x = srat@meta.data$id_cell, from = barcode2celltype_df$id_cell, to = as.vector(barcode2celltype_df$id_bycluster_bycellgroup_byaliquot))
Idents(srat) <- "id_bycluster_bycellgroup_byaliquot" 
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "avgexp.", assay_process, ".bycellgroup.byaliquot.bycluster.", "31_aliquot_integration.", run_id, ".tsv")
