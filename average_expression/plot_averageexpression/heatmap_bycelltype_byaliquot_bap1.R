...
# write data matrix -------------------------------------------------------
file2write <- paste0(dir_out, "bap1_input_data_matrix.csv")
write.csv(x = plot_data_mat, file = file2write)

# make column split -------------------------------------------------------
idaliquot_bap1_germline <- bulk_sn_omicsprofile_df$Aliquot.snRNA[!is.na(bulk_sn_omicsprofile_df$Mut.BAP1.Germline) & (bulk_sn_omicsprofile_df$Mut.BAP1.Germline != "None")]
idaliquot_bap1_germline
idaliquot_3pdel_only <- bulk_sn_omicsprofile_df$Aliquot.snRNA[!is.na(bulk_sn_omicsprofile_df$Mut.BAP1) & (bulk_sn_omicsprofile_df$Mut.BAP1 == "None")]
columnsplit_vec <- ifelse(ids_aliquot %in% idaliquot_bap1_germline, "BAP1_Germline",
                          ifelse(ids_aliquot %in% idaliquot_3pdel_only, "3p_Loss", "BAP1_Somatic\n+3p_Loss"))
columnsplit_factor <- factor(x = columnsplit_vec, levels = c("BAP1_Germline", "BAP1_Somatic\n+3p_Loss", "3p_Loss"))
...
heatmapbody_color_fun <- colorRamp2(c(quantile(plot_data_mat, 0.025, na.rm=T),
                                    ...
                                    c("white", 
                                      RColorBrewer::brewer.pal(n = 9, name = "YlGn")[6],
                                      RColorBrewer::brewer.pal(n = 9, name = "YlGn")[9]))
...
# save output -------------------------------------------------------------
file2write <- paste0(dir_out, gene_plot, "_expression.", "across_celltypes.", run_id, ".png")
png(filename = file2write, width = 1500, height = 1200, res = 150)
draw(object = p, 
     annotation_legend_side = "right", annotation_legend_list = annotation_lgd)
dev.off()

file2write <- paste0(dir_out, gene_plot, "_expression.", "across_celltypes.", run_id, ".pdf")
