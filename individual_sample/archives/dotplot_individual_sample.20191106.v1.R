...
for (snRNA_aliquot_id_tmp in snRNA_aliquot_ids) {
  ...
  ## plot SMG gene expression
  genes2plot <- SMGs[["CCRCC"]]
  p <- DotPlot(object = seurat_obj, features = genes2plot, col.min = 0)
  p$data$cluster_cell_type <- plyr::mapvalues(p$data$id, from = cluster2celltype_tab_tmp$Cluster, to = cluster2celltype_tab_tmp$Enriched_Cell_Type_Abbr)
  p <- p + facet_grid(cluster_cell_type~., scales = "free", space = "free", drop = T)
  p <- p  + RotatedAxis()
  p <- p + theme(panel.spacing = unit(0, "lines"),
                 strip.background = element_blank(),
                 panel.border = element_rect(colour = "black"),
                 strip.text.y = element_text(angle = 0, vjust = 0.5, size = 12, face = "bold"),
                 axis.text.x = element_text(size = 14, face = "bold"),
                 strip.placement = "outside")
  
  file2write <- paste0(dir_out, snRNA_aliquot_id_tmp,".Individual_Clustered_Dotplot_Cell_SMG_Exp.", run_id, ".png")
  png(file = file2write, width = 1200, height = 1000, res = 150)
  print(p)
  dev.off()
  
  ## plot HIF gene expression
  genes2plot <- c("HIF1A", "ARNT", "EPAS1", "ARNT2")
  p <- DotPlot(object = seurat_obj, features = genes2plot, col.min = 0)
  p$data$cluster_cell_type <- plyr::mapvalues(p$data$id, from = cluster2celltype_tab_tmp$Cluster, to = cluster2celltype_tab_tmp$Enriched_Cell_Type_Abbr)
  p <- p + facet_grid(cluster_cell_type~., scales = "free", space = "free", drop = T)
  p <- p  + RotatedAxis()
  ...
  file2write <- paste0(dir_out, snRNA_aliquot_id_tmp,".Individual_Clustered_Dotplot_Cell_HIF_Exp.", run_id, ".png")
