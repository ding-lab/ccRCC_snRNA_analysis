...
genesets_test <- c("HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION", "HALLMARK_GLYCOLYSIS", "HALLMARK_OXIDATIVE_PHOSPHORYLATION" )
...
for (i in 1:ncol(pairwise)) {
  ...
  plotdata_df$cluster_name <- as.numeric(plotdata_df$cluster) + 1
  
  geneset_x_plot <- gsub(x = geneset_x, pattern = "HALLMARK_", replacement = "")
  geneset_y_plot <- gsub(x = geneset_y, pattern = "HALLMARK_", replacement = "")
  genesets_plot <- c(geneset_x_plot, geneset_y_plot)
  genesets_plot[genesets_plot == "OXIDATIVE_PHOSPHORYLATION"] <- "OXPHOS"
  genesets_plot[genesets_plot == "EPITHELIAL_MESENCHYMAL_TRANSITION"] <- "EMT"
  
  # plot --------------------------------------------------------------------
  p <- ggscatter(data = plotdata_df, x = "x_plot", y = "y_plot",
                 add = "reg.line",  # Add regressin line
                 # label = "cluster_name", font.label = c(14, "plain"),
                 add.params = list(color = "grey", fill = "lightgray", linetype = 2)
  )
  p <- p + stat_cor(method = "pearson",
                    label.x = min(plotdata_df$x_plot),
                    label.y = (max(plotdata_df$y_plot)+0.04), size = 7)
  p <- p + geom_text_repel(mapping = aes(label = cluster_name), min.segment.length = 0, box.padding = 0.5, segment.alpha = 0.5, segment.size = 0.2, size = 6)
  # p <- p + ggtitle(label = paste0(protein_tmp, "~", treatment_tmp, " 1-month response"))
  p <- p + theme_classic(base_size = 17)
  p <- p + xlab(paste0(genesets_plot[1], " score"))
  p <- p + ylab(paste0(genesets_plot[2], " score"))
  # p <- p + xlim(c(min(plotdata_tmp_df$x_plot)-0.11, max(plotdata_tmp_df$x_plot)+0.11))
  p <- p + ylim(c(min(plotdata_df$y_plot)-0.05, max(plotdata_df$y_plot)+0.05))
  ...
  # print(p)
  # dev.off()
  file2write <- paste0(dir_out, geneset_x, ".vs.", geneset_y, ".pdf")
