...
scores_df <- fread(data.table = F, input = "./Resources/Analysis_Results/signature_scores/process_signature_scores/getmedian_visionscore_bysiggeneset_byintrapatientcluster/20220606.v1/median_vision_score.sig_genesets.byintrapatient_tumor_cluster.20220606.v1.tsv")
...
scoregroup_tmp <- "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"
pvalues_vec <- NULL
for (scoregroup_tmp in "EPITHELIAL_MESENCHYMAL_TRANSITION") {
  ## make plot data
  scores_tmp_df <- scores_df[, c("intrapatient_cluster_name", scoregroup_tmp)]
  ...
  pvalues_vec <- c(pvalues_vec, test_result$p.value)
}
wilcox_df <- data.frame(name_score = scoregroups_test, fdr = p.adjust(p = pvalues_vec, method = "fdr"), pvalue = pvalues_vec)

# save output -------------------------------------------------------------
file2write <- paste0(dir_out, "wilcox_results.tsv")
write.table(x = wilcox_df, file = file2write, quote = F, sep = "\t", row.names = F)
