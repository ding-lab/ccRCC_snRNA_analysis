...
# deg_filtered_df <- fread(data.table = F, input = "./Resources/snRNA_Processed_Data/Differentially_Expressed_Genes/PBRM1_mutants_tumor-up_TF_DEG_withMotifs_inPromoterDACR.20201227.tsv")
deg_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/tumor_vs_normal/summarize_deg/summarize_sig_up_deg_avgLogFC_across_snatac_tumors/20201223.v3/DEGs_sig_up.20201223.v3.tsv")
## get biomart
...
easyids <- colnames(deg_df)[grepl(pattern = "C3", x = colnames(deg_df))]
deg_df <- deg_df %>%
  filter(count_nonmutanttumor_up<2 & count_pbrm1tumor_up>=2 & count_bap1tumor_up<2)
...
## get gene list
de_genes <- rowMeans(deg_df[,easyids], na.rm = T)
de_genes
...
enricher_out <- tryCatch(expr = enricher(gene = names(de_genes), TERM2GENE = wpid2gene, TERM2NAME = wpid2name, pvalueCutoff = 1, qvalueCutoff = 1),
