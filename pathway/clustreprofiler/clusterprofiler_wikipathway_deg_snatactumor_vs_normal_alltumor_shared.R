...
deg_df <- deg_anno_df %>%
  filter(category_byshared %in% c("TRUE_TRUE_TRUE"))
...
## get gene list
de_genes <- deg_df$mean_avg_logFC.nonmutant
de_genes
...
# test over-representation analysis and gene set enrichment using wikipathway ------------------------------
enricher_out <- tryCatch(expr = enricher_out <- GSEA(geneList = de_genes, TERM2GENE = wpid2gene, TERM2NAME = wpid2name, verbose=FALSE, pvalueCutoff = 1, qvalueCutoff = 1),
