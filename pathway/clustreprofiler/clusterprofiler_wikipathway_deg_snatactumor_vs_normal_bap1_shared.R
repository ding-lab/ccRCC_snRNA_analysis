...
deg_df <- deg_anno_df %>%
  filter(category_byshared %in% c("FALSE_TRUE_FALSE"))
...
# test over-representation analysis and gene set enrichment using wikipathway ------------------------------
enricher_out <- tryCatch(expr = enricher_out <- GSEA(geneList = de_genes, TERM2GENE = wpid2gene, TERM2NAME = wpid2name, pvalueCutoff = 1),
