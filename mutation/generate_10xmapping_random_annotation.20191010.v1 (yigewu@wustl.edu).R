...
seurat_summary <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/snRNA_Processed_Data/scRNA_auto/summary/RCC_snRNA_Downstream_Processing - Seurat_Summary.09272019.v5.tsv", data.table = F)
seurat_summary2process <- seurat_summary %>%
  filter(Cellranger_reference_type == "pre-mRNA") %>%
  filter(Proceed_for_downstream == "Yes") %>%
  filter(Aliquot %in% aliquot_ids) %>%
...
for (i in 1:nrow(seurat_summary2process)) {
  aliquot_id_tmp <- seurat_summary2process$Aliquot[i]
  ...
  write.table(x = anno_tab_tmp, file = paste0(dir_out, aliquot_id_tmp, facs_tmp, "_AfterQC_Barcodes.tsv"), quote = F, row.names = F, sep = "\t", col.names = F)
