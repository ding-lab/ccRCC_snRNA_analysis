...
# for (snRNA_aliquot_id_tmp in unique(paths_srat_df$Aliquot)) {
  ...
  if (snRNA_aliquot_id_tmp == "CPT0000890002") {
    next()
  }
  ...
  plot_data_df <- merge(umap_tab, mutation_map_tab.var, by = "barcode", all.x = T)
  ...
  p <- p + geom_text_repel(data = plot_data_df[plot_data_df$read_type == "Var",], mapping = aes(UMAP_1, UMAP_2, label = gene_symbol))
