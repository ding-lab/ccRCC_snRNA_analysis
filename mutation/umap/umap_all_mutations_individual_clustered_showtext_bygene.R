...
for (snRNA_aliquot_id_tmp in unique(umap_df$aliquot)) {
  ...
  mutation_map_tab.var.all <- snRNA_mutation_df %>%
    filter(aliquot == snRNA_aliquot_id_tmp) %>%
    filter(Is_Silent == F) %>%
    filter(allele_type == "Var")
  
  for (gene_tmp in unique(mutation_map_tab.var.all$gene_symbol)) {
    ...
    p
    file2write <- paste0(dir_out_tmp, aliquot_show, "_", gene_tmp, ".png")
