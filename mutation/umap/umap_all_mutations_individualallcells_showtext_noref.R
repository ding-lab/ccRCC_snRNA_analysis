...
# for (snRNA_aliquot_id_tmp in unique(umap_df$aliquot)) {
  ...
  p <- p + geom_point_rast(data = plot_data_df[plot_data_df$read_type == "Var" & !(plot_data_df$gene_symbol %in% ccRCC_SMGs),], mapping = aes(x = UMAP_1, y = UMAP_2), alpha = 0.5, size = 0.8, color = colors_read_type["cells with the variant read(s)"])
  p <- p + geom_point_rast(data = plot_data_df[plot_data_df$read_type == "Var" & (plot_data_df$gene_symbol %in% ccRCC_SMGs),], mapping = aes(x = UMAP_1, y = UMAP_2), alpha = 0.8, size = 1, color = colors_read_type["cells with the variant read(s)"])
  p <- p + geom_text_repel(data = plot_data_df[!is.na(plot_data_df$gene_symbol),], 
                           mapping = aes(UMAP_1, UMAP_2, label = gene_symbol, size = Driver_Gene_Mutation), color = "black")
  # p <- p + scale_color_manual(values = c("TRUE" = "black", "FALSE" = "grey50"))
  p <- p + scale_size_manual(values = c("TRUE" = 4, "FALSE" = 4))
  p <- p +
    theme_bw() +
  ...
  p
  file2write <- paste0(dir_out_tmp, aliquot_show, ".mut.png")
  png(file2write, width = 800, height = 800, res = 150)
  print(p)
  dev.off()
  file2write <- paste0(dir_out_tmp, aliquot_show, ".mut.pdf")
  ...
  p <- p + ggplot2::theme(axis.line=element_blank(),axis.text.x=element_blank(),
                          ...
                          axis.title.y=element_blank())
  p
  file2write <- paste0(dir_out_tmp, aliquot_show, ".mut.legend.png")
  png(file2write, width = 800, height = 900, res = 150)
  print(p)
  dev.off()
  file2write <- paste0(dir_out_tmp, aliquot_show, ".mut.legend.pdf")
