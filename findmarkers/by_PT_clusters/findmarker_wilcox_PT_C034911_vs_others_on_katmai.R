...
group1_findmarkers <- "PT cluster0,3,4,9,11"
group2_findmarkers <- "other PT clusters"
...
barcode2celltype_df <- barcode2celltype_df %>%
  mutate(group_findmarkers = ifelse(cell_group %in% paste0("PT_C", c(0,3,4,9,11)), 
                                    group1_findmarkers, 
                                    ifelse(cell_group %in% paste0("PT_C", c(5,8,13,15)), group2_findmarkers, "Others")))
## map group label
srat@meta.data$group_findmarkers <- mapvalues(x = srat@meta.data$id_aliquot_barcode, from = barcode2celltype_df$barcode, to = as.vector(barcode2celltype_df$group_findmarkers), warn_missing = F)
...
## write output
file2write <- paste0(dir_out, "PT.Byclustergroup.", test_process, 
