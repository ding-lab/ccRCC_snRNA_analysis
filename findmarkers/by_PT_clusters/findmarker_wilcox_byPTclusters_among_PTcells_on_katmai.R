...
## map group label
srat@meta.data$group_findmarkers <- mapvalues(x = srat@meta.data$id_aliquot_barcode, from = barcode2celltype_df$barcode, to = as.vector(barcode2celltype_df$cell_group), warn_missing = F)
Idents(srat) <- "group_findmarkers" 
dim(srat)
srat <- subset(srat, idents = paste0("PT_C", c(0,3,4,9,11, 5,8,13,15)))
dim(srat)

## run findmarkers
deg_df <- FindAllMarkers(object = srat, test.use = test_process, only.pos = F,
