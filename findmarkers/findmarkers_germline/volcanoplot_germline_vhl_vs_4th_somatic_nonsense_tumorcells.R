...
# input denpendencies -----------------------------------------------------
## input marker table
markers_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/findmarkers_germline/findallmarker_wilcox_germline_vhl_vs_4th_somatic_nonsense_bycelltype_on_katmai/20200604.v1/VHL_Germline_vs_VHL_Somatic.FindMarkers.Wilcox.20200604.v1.tsv")
## input ppi table
...
# for (celltype_plot in c("Tumor cells", "Macrophages")) {
  ...
  p <- p + geom_text_repel(data = subset(plot_data_df, genegroup == "P.adjusted<0.05, Known to interact with VHL" & (logPvalue > 100 | deg_gene_symbol == "MYC")),
                           mapping = aes(x = avg_logFC, y = y_plot, label = deg_gene_symbol), color = "red", force = 2, box.padding = unit(0.5, "lines"))
  p <- p + scale_color_manual(values = colors_genegroup)
  p <- p + xlim(c(-1,1))
  p <- p + ylim(c(0, max_logp))
  p <- p + ggtitle(label = paste0("Differentially Expressed Genes in ", celltype_plot), subtitle = paste0("One Germline-VHL-Mutated Sample vs\nOne Somatic-VHL-Mutated Sample ", "(", "CPT0086820004", ")"))
  ...
  # save output -------------------------------------------------------------
  file2write <- paste0(dir_out, "volcanoplot.4th_somatic_samples", ".pdf")
  pdf(file = file2write, width = 6, height = 6)
  print(p)
  dev.off()
  
  file2write <- paste0(dir_out, "volcanoplot.4th_somatic_samples", ".png")
