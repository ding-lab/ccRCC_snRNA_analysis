...
plot_data_df <- deg_df %>%
  ...
  mutate(foldchange_type = ifelse(FDR.cnvcorrected < 0.05, ifelse(avg_lo2FC.alltumorcells > 0  & Num_down == 0 & Num_sig_up >= 5, "consistently higher in BAP1-mutants",
                                  ifelse(avg_lo2FC.alltumorcells < 0  & Num_up == 0 & Num_sig_down >= 5, "consistently lower in BAP1-mutants",
                                         ifelse(Num_up+Num_down >= 5, "mixed fold change directions", "<50% samples w. sig. fold changes"))), "<50% samples w. sig. fold changes")) %>%
...
p <- p + geom_point(data = subset(plot_data_df, foldchange_type %in% c("consistently higher in BAP1-mutants", "consistently lower in BAP1-mutants")), mapping = aes(x = x_plot, y = y_plot, size = size_plot, color = foldchange_type), alpha = 0.8, shape = 16)
...
# p <- p + geom_point(data = subset(plot_data_df, foldchange_type == "Consistently higher in BAP1-mutants"), mapping = aes(x = x_plot, y = y_plot, size = size_plot), alpha = 0.8, color = color_red)
# p <- p + geom_point(data = subset(plot_data_df, foldchange_type == "Consistently lower in BAP1-mutants"), mapping = aes(x = x_plot, y = y_plot, size = size_plot), alpha = 0.8, color = color_blue)
p <- p + scale_color_manual(values = c("consistently higher in BAP1-mutants" = color_red, 
                                       "consistently lower in BAP1-mutants" = color_blue, 
                                       "mixed fold change directions" = color_purple,
                                       "<50% samples w. sig. fold changes" = "grey50"))
...
p <- p + scale_size_area(max_size = 4)
p <- p + theme_classic()
p <- p + xlab("Log2 fold change sn gene expression\n(10 BAP1-mutant tumor vs. 19 other tumors)")
p <- p + ylab("-Log10FDR")
p <- p + ylim(c(0, 450)) + xlim(c(-2.5, 2.5))
