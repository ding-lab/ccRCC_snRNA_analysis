...
dir_out1 <- "./Resources/snRNA_Processed_Data/Differentially_Expressed_Genes/BAP1_vs_PBRM1_NonMutants_Tumorcells/"
dir.create(dir_out1)
...
# preprocess ----------------------------------------
cases_bap1 <- mut_df$Case[mut_df$mutation_category_sim %in% c("Both mutated", "BAP1 mutated")]
...
aliquots_group1
cases_group2 <- mut_df$Case[mut_df$mutation_category_sim %in% c("Non-mutants", "PBRM1 mutated")]
  ...
  if (!file.exists(file2write)) {
    ...
    barcode2celltype_df <- barcode2celltype_df %>%
      ...
      mutate(group_findmarkers = ifelse(Cell_group5 == "Tumor cells" & orig.ident == aliquot_group1_tmp, 
                                        group1_findmarkers, 
                                        ifelse(Cell_type.detailed == "Tumor cells" & orig.ident %in% aliquots_group2, group2_findmarkers, "Others")))
    ## map group label
