...
for (cna_gene_plot in names(oraresult_list)[!(names(oraresult_list) %in% c("JAK2", "QKI"))]) {
  ## set output directory
  ...
  ## extract plot object
  object_plot <- oraresult_list[[cna_gene_plot]]
  if (nrow(object_plot@result) > 0) {
    p <- heatplot(x = object_plot)
    p <- p + ggtitle(label = paste0("ORA for Differentially Expressed Genes Associated with ", cna_gene_plot, " ", cna_gene_expected_state))
    # p
    file2write <- paste0(dir_out_sub, cna_gene_plot, ".ORA.dotplot.png")
    png(filename = file2write, res = 150, width = 5000, height = 900)
    print(p)
    dev.off()
  }
}
...
for (cna_gene_plot in names(gsearesult_list)) {
  ...
  if (nrow(object_plot@result) > 0) {
    p <- heatplot(x = object_plot, foldChange = object_plot@geneList)
    p <- p + ggplot2::scale_fill_gradient2(high = "red",
                                           mid = "white",
                                           low = "blue",
                                           midpoint = 0)
    p <- p + ggtitle(label = paste0("GSEA for Differentially Expressed Genes Associated with ", cna_gene_plot, " ", cna_gene_expected_state))
    # p
