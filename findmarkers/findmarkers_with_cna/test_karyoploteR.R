...
genesymbol2ucsc_df <- getBM(attributes=c('entrezgene_id', 'hgnc_symbol'), 
                            filters = 'hgnc_symbol', 
                            values = markers_for_cnagene_df$de_gene_symbol, 
                            mart = ensembl)
## add entrez ids to the deg table
markers_for_cnagene_df$entrezgene_id <- mapvalues(x = markers_for_cnagene_df$de_gene_symbol, from = genesymbol2ucsc_df$hgnc_symbol, to = as.vector(genesymbol2ucsc_df$entrezgene_id))
## filter markers by entrez id
markers_for_cnagene_df <- markers_for_cnagene_df %>%
...
head(ordered, n=20)
## get top genes
top.genes <- ordered[ordered$num_aliquot_de == 3]
top.genes
# plot --------------------------------------------------------------------
## get plot parameters
fc.ymax <- round(max(abs(range(filtered.hs.genes$mean_avg_logFC))), digits = 0)
fc.ymin <- -fc.ymax
