...
## input CNV profile
cnv_bycase_df <- fread(data.table = F, input = "./Resources/Analysis_Results/bulk/copy_number/write_sample_bicseq_cnv_profile/20210503.v1/Bulk_WGS_Chr_CNV_Profile.20210503.v1.tsv")
...
latent.vars_process <- c("Is.5q.Gain")
...
## add case id
srat@meta.data$id_case <- mapvalues(x = srat@meta.data$orig.ident, from = idmetadata_df$Aliquot.snRNA, to = as.vector(idmetadata_df$Case))
## add 5q status
srat@meta.data$CN.5q <- mapvalues(x = srat@meta.data$id_case, from = cnv_bycase_df$Case, to = as.vector(cnv_bycase_df$CN.5q))
srat@meta.data$Is.5q.Gain <- ifelse(srat@meta.data$CN.5q == "gain", 1, 0); srat@meta.data$Is.5q.Gain <- as.numeric(as.vector(srat@meta.data$Is.5q.Gain))
head(srat@meta.data)
...
deg_df <- FindMarkers(object = srat, test.use = test_process, ident.1 = group1_findmarkers, ident.2 = group2_findmarkers, only.pos = F,
                      ...
                      latent.vars = latent.vars_process)
