...
deg_long_df <- deg_df %>%
  filter(easyid_tumor != "C3L-00359-T1") 
deg_wide_df2 <- reshape2::dcast(data = deg_long_df, formula = genesymbol_deg~easyid_tumor, value.var = 'avg_logFC', na.rm = T)
...
deg_wide_df3 <- reshape2::dcast(data = deg_sig_long_df, formula = genesymbol_deg~easyid_tumor, value.var = 'avg_logFC', na.rm = T)
...
deg_wide_df <- deg_wide_df[, c("genesymbol_deg", "Num_sig_up", "Num_sig_down", "mean_avg_logFC", "Num_datapoints", "Num_up", "Num_down", easyids_tumor)]
