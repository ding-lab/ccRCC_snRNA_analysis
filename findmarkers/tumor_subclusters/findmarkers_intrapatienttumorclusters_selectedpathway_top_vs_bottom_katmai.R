...
for (geneset_tmp in c("UNFOLDED_PROTEIN_RESPONSE_Score", "TGF_BETA_SIGNALING_Score", "TNFA_SIGNALING_VIA_NFKB_Score",
  ...
  deg_df$geneset <- geneset_tmp
  deg_all_df <- rbind(deg_all_df, deg_df)
}

file2write <- paste0(dir_out, "selectedpathways", ".top_vs_bottom.", 
                     ".logfc.threshold", logfc.threshold.run, 
                     ".min.pct", min.pct.run,
                     ".min.diff.pct", min.diff.pct.run,
                     ".Assay", assay_process,
                     ".tsv")
write.table(x = deg_all_df, file = file2write, sep = "\t", quote = F, row.names = F)
cat("finish writing the result!\n\n")
