...
# find all markers --------------------------------------------------------
markers_wilcox_df <- FindMarkers(object = srat, test.use = "wilcox", only.pos = F, min.pct = min.pct.wilcox, logfc.threshold = logfc.threshold.wilcox, verbose = T, ident.1 = 0)
markers_wilcox_df$rowname <- rownames(markers_wilcox_df)

# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "C3N-00010.correlatedtumorcells.FindAllMarkers.bynewcluster0.Wilcox.Minpct", min.pct.wilcox, ".Logfc", logfc.threshold.wilcox,".tsv")
