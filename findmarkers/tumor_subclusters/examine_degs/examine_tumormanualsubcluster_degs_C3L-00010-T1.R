...
deg_sample_df <- deg_df %>%
  ...
  unique()
deg_sample_df %>%
  select(ident.1, ident.2) %>%
  unique()
# filter ppit -------------------------------------------------------------
ppi_gene_filtered_df <- ppi_pair_df %>%
  filter(GENE %in% c("RB1", "IDH2", "ARID1A", "PBRM1", "BAP1", "SETD2", "PRKCI", "MECOM",  "GOLPH3", "ARID1B", "JAK2"))

# annotate degs -----------------------------------------------------------
deg_sample_filtered_df <- merge(deg_sample_df %>%
                                  filter(p_val < 0.05), ppi_gene_filtered_df, by.x = c("deg_gene_symbol"), by.y = c("SUB_GENE"))
