...
genes_filter <- unique(c(emtgenes_df$hgnc_symbol[emtgenes_df$gene_function == "Mesenchymal"]))
genes_filter
...
p <- p + geom_text_repel(data = subset(plot_data_df, !is.na(text_gene)),
                         mapping = aes(x = x_plot, y = y_plot, label = text_gene), color = "black", force = 4, alpha = 0.8)
