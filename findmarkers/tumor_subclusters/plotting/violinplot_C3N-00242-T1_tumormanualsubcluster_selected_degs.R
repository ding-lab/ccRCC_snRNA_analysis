...
## input selected deg
genes2plot_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/tumor_subclusters/examine_degs/examine_tumormanualsubcluster_degs_C3N-00242-T1/20200629.v2/C3N-00242-T1.DEG_selected.tsv")
## set sample id
...
id_aliquot
## set genes 2 plot
genes2plot <- unique(genes2plot_df$deg_gene_symbol)
...
for (gene_tmp in genes2plot) {
  p <- VlnPlot(object = srat, features = gene_tmp, group.by = "Name_Cluster", idents = c("C1", "C2"), pt.size = 0.25)
