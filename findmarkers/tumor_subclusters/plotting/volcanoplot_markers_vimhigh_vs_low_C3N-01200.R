...
emtgenes_df <- fread(data.table = F, input = "./Resources/Analysis_Results/dependencies/write_emt_genes/20200910.v1/EMT_Genes.20200910.v1.tsv")
...
genes_filter <- unique(c(emtgenes_df$hgnc_symbol, gene2celltype_df$Gene[gene2celltype_df$Cell_Type1 == "Proximal tubule"]) )
genes_filter
...
p <- p + geom_text_repel(data = subset(plot_data_df, !is.na(text_gene)),
                         mapping = aes(x = x_plot, y = y_plot, label = text_gene), color = "black", force = 2, alpha = 0.8, fontface = "bold")
