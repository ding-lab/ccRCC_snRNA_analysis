...
genes_filter <- unique(c(emtgenes_df$hgnc_symbol[emtgenes_df$gene_function != "Unknown"], gene2celltype_df$Gene[gene2celltype_df$Cell_Type1 %in% c("Proximal tubule", "Tumor cells")]) )
genes_filter
...
p <- p + xlim(c(-3, 3))
p <- p + xlab("log(Fold-Change) (transitional cells vs tumor cells)")
