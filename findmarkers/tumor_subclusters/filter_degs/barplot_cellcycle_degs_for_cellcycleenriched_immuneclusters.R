...
# input dependencies ------------------------------------------------------
count_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/tumor_subclusters/filter_degs/count_cellcycle_degs_for_cellcycleenriched_tumorclusters/20210427.v1/Count.Cellcycle_DEGs.tsv")

# prepare data ------------------------------------------------------------
plot_data_df <- count_df %>%
  filter(Category_sim != "") %>%
  filter(count_clusters >= 13/3)
