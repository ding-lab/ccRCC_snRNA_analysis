...
cutoff_emt <- quantile(x = enrich_df$EPITHELIAL_MESENCHYMAL_TRANSITION_Score, probs = 0.9)
clusternames_filter <- enrich_df$cluster_name2[enrich_df$EPITHELIAL_MESENCHYMAL_TRANSITION_Score >= cutoff_emt]
## map easy id
...
## get genes to filter
genes_filter <- gene2pathway_df$GeneSymbol[gene2pathway_df$GeneSet_Name == "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"]
## filter
