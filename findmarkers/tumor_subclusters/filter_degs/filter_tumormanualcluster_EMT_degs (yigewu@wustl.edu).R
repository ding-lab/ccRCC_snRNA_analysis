...
# filter epithelial & PT related genes ------------------------------------
genes_pt_epi <- unique(emt_genes_df$Gene[emt_genes_df$Gene_Group2 %in% c("Epithelial", "Proximal tubule")])
genes_pt_epi <- unique(c(genes_pt_epi, gene2celltype_df$Gene[gene2celltype_df$Cell_Type1 == "Proximal tubule" & !(gene2celltype_df$Cell_Type2 %in% c("PT-A", "PT-B", "PT-C"))]))
genes_pt_epi <- genes_pt_epi[!(genes_pt_epi %in% c("VCAM1"))]
deg_epithelia_df <- deg_sig_df %>%
  filter(avg_log2FC < 0 ) %>%
  filter(genesymbol_deg %in% genes_pt_epi) %>%
