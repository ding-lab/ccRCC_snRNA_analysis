...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "BAP1_vs_NonMutants.DEGs.Chromosome_Regions.", run_id, ".tsv")
write.table(x = genesymbol2region_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "BAP1_vs_NonMutants.DEGs.Chromosome_Regions_Annotated.", run_id, ".tsv")
