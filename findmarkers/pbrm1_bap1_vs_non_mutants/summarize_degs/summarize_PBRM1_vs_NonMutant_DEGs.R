...
# input dependencies ------------------------------------------------------
deg_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/pbrm1_bap1_vs_non_mutants/summarize_degs/unite_PBRM1_vs_NonMutant_DEGs/20210624.v1/PBRM1_vs_NonMutants_DEGs.20210624.v1.tsv")

# summarize genes by occurance ---------------------------------------------
deg_sig_long_df <- deg_df %>%
  filter(group1_mut_category %in% c("PBRM1 mutated")) %>%
...
cutoff_pbrm1_vs_others <- 0.5*length(easyids_tumor)
deg_wide_df <- deg_wide_df %>%
  mutate(PBRM1_vs_OtherTumor_snRNA = ifelse(Num_sig_up >= cutoff_pbrm1_vs_others, "Up",
                                           ifelse(Num_sig_down >= cutoff_pbrm1_vs_others, "Down", "Inconsistent")))
