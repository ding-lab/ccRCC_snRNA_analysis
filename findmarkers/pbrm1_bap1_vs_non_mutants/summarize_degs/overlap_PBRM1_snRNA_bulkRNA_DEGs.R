...
# input dependencies ------------------------------------------------------
deg_snRNA_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/pbrm1_bap1_vs_non_mutants/summarize_degs/summarize_PBRM1_BAP1_DEGs/20210405.v1/PBRM1_DEGs.20210405.v1.tsv")
...
deg_merged_filtered_df <- deg_merged_df %>%
  filter(!is.na(PBRM1_Mutated_vsOthers.bulkRNA) & PBRM1_vs_BAP1_Mutants_snRNA == PBRM1_Mutated_vsOthers.bulkRNA)
deg_merged_filtered_df <- deg_merged_filtered_df %>%
  arrange(desc(PBRM1_vs_BAP1_Mutants_Tumorcells_Up), desc(PBRM1_vs_BAP1_Mutants_Tumorcells_Up), desc(PBRM1_vs_BAP1_Mutants_Tumorcells_Up))
table(deg_merged_filtered_df$PBRM1_vs_BAP1_Mutants_snRNA)

# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "PBRM1_DEGs.", "snRNA_and_bulkRNA.", run_id, ".tsv")
