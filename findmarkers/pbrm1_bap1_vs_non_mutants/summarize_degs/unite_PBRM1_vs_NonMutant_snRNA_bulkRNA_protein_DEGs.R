...
deg_snRNA_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/pbrm1_bap1_vs_non_mutants/summarize_degs/summarize_PBRM1_vs_NonMutant_DEGs/20210624.v1/PBRM1_DEGs.Sig.20210624.v1.tsv")
### input the CNV correction results
deg_cnvcor_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/pbrm1_bap1_vs_non_mutants/findmarker_LR_wCNV_all_PBRM1_tumorcells_vs_NonMutant_cells_on_katmai/20210624.v1/LR.logfc.threshold0.min.pct0.1.min.diff.pct0.AssayRNA.tsv")
### input overall fold change
deg_snRNA_fc_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/pbrm1_bap1_vs_non_mutants/findmarker_LR_all_PBRM1_tumorcells_vs_NonMutant_cells_on_katmai/20210624.v1/LR.logfc.threshold0.min.pct0.1.min.diff.pct0.AssayRNA.tsv")
## input bulk RNA DEGs
...
deg_bulkprotein_df <- fread(data.table = F, input = "./Resources/Analysis_Results/bulk/expression/protein/compare_bulk_protein_PBRM1_tumor_vs_NonMutant_tumors/20210625.v1/Bulk_Protein_PBRM1_vs_NonMutant.Wilcox.20210625.v1.tsv")
...
deg_bulkRNA_df2 <- deg_bulkRNA_df %>%
  ...
  filter(comparison == "PBRM1_Mutated_vs_NonMutants") %>%
...
deg_bulkprotein_df <- deg_bulkprotein_df %>%
  ...
  rename(number_group1.bulkpro = number_group1) %>%
  rename(number_group1.bulkpro = number_group2) %>%
...
deg_filtered_df1 <- deg_merged_df %>%
  ...
  mutate(foldchange_type = ifelse(FDR.snRNA.cnvcorrected < 0.05, ifelse(avg_log2FC.snRNA > 0  & Num_down.snRNA == 0 & Num_sig_up.snRNA >= 5, "consistently higher in PBRM1-mutants",
                                                                        ifelse(avg_log2FC.snRNA < 0  & Num_up.snRNA == 0 & Num_sig_down.snRNA >= 5, "consistently lower in PBRM1-mutants",
...
deg_filtered_df2 <- deg_filtered_df1 %>%
  filter(foldchange_type %in% c("consistently higher in PBRM1-mutants", "consistently lower in PBRM1-mutants"))
