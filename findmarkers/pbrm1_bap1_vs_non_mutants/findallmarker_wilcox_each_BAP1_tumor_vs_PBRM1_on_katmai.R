...
dir_out <- "./Resources/snRNA_Processed_Data/Differentially_Expressed_Genes/BAP1_vs_PBRM1_Mutants_Tumorcells/"
dir.create(dir_out)
...
group1_findmarkers <- "BAP1-mutant Tumor cells"
group2_findmarkers <- "PBRM1-mutant Tumor cells"

# preprocess ----------------------------------------
cases_group1 <- mut_df$Case[mut_df$mutation_category_sim == "BAP1 mutated"]
...
aliquots_group1
cases_group2 <- mut_df$Case[mut_df$mutation_category_sim == "PBRM1 mutated"]
...
for (aliquot_group1_tmp in aliquots_group1) {
  ...
  file2write <- paste0(dir_out,
                       easyid_group1_tmp, ".vs_PBRM1Mutants_Tumorcells.tsv")
  if (!file.exists(file2write)) {
    ...
    barcode2celltype_df <- barcode2celltype_df %>%
      ...
      mutate(group_findmarkers = ifelse(Cell_group5 == "Tumor cells" & orig.ident == aliquot_group1_tmp, 
                                        "BAP1-mutant Tumor cells", 
                                        ifelse(Cell_type.detailed == "Tumor cells" & orig.ident %in% aliquots_group2, "PBRM1-mutant Tumor cells", "Others")))
    ## map group label
