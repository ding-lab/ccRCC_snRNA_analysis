...
druggenes_df <- fread(data.table = F, input = "./Resources/Knowledge/Gene_Lists/01-Jul-2021-GeneSummaries.tsv")

# filter ------------------------------------------------------------------
markers_merged_df <- merge(x = markers_df, y = druggenes_df, by.x = c("Gene"), by.y = c("name"), all.x = T)
markers_merged_df <- markers_merged_df %>%
  arrange(gene_civic_url)
