...
### find anchors
renal.anchors <- FindIntegrationAnchors(object.list = renal.list, dims = 1:30)
saveRDS(object = renal.anchors, file = paste0(dir_out, "30_aliquot_integration.anchors.", run_id, ".RDS"), compress = T)
rm(renal.list)
...
## save as RDS file
saveRDS(object = renal.integrated, file = paste0(dir_out, "30_aliquot_integration.anchored.", run_id, ".RDS"), compress = T)
...
png(file = file2write, width = 2000, height = 1600, res = 150)
print(p)
dev.off()
...
png(file = file2write, width = 3000, height = 2000, res = 150)
print(p)
dev.off()
