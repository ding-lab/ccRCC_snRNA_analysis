...
cluster2celltype_df <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/snRNA_Processed_Data/Cell_Type_Assignment/integration.allcluster2celltype.20200213.v3.tsv", data.table = F)
## input meta data
...
barcodes_by_aliquot_by_cluster_df <- barcode2cluster_df %>%
  dplyr::select(orig.ident, ident) %>%
  dplyr::rename(aliquot = orig.ident) %>%
...
## write table
write.table(x = barcodes_by_aliquot_by_cluster_df, file = paste0(dir_out, "Cell_Group_Composition.by_aliquot_by_cluster", run_id, ".tsv"), quote = F, row.names = F, sep = "\t")

# make barplot colored by aliquots------------------------------------------------------------
plot_df <- barcodes_by_aliquot_by_cluster_df
## get fill color palette
fill_colors <- Polychrome::palette36.colors(n = 30)
names(fill_colors) <- unique(plot_df$aliquot)
p <- ggplot()
...
## save barplot
file2write <- paste0(dir_out, "30_aliquot_integration.20200211.v3.Cell_Group_Composition.by_aliquot_by_cluster.", run_id, ".png")
png(file = file2write, width = 3000, height = 1000, res = 150)
print(p)
dev.off()

# make barplot colored by case------------------------------------------------------------
plot_df <- barcodes_by_aliquot_by_cluster_df
## get fill color palette
fill_colors <- Polychrome::dark.colors(n = 24)
names(fill_colors) <- unique(plot_df$case)
p <- ggplot()
p <- p + geom_col(data = plot_df, mapping = aes(x = cluster, y = Frac_by_aliquot_by_cluster, fill = case), color = "black", position = "stack")
p <- p + facet_grid(.~Most_Enriched_Cell_Group, scales = "free", space = "free", drop = T)
p <- p + scale_fill_manual(values = fill_colors)
p <- p + theme(panel.spacing = unit(0, "lines"),
               axis.text.x = element_text(size = 12, vjust = 0.5),
...
## save barplot
file2write <- paste0(dir_out, "30_aliquot_integration.20200211.v3.Cell_Group_Composition.by_case_by_cluster.", run_id, ".png")
png(file = file2write, width = 3000, height = 1000, res = 150)
print(p)
dev.off()
