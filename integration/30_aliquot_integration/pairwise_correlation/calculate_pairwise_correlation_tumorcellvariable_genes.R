...
avg.exp.mat <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/integration/30_aliquot_integration/averageexpression/averageexpression_tumor_cells/20200309.v1/30_aliquot_integration.tumor_cells.averageexpression.20200309.v1.tsv", data.table = F)
...
spearman_rhos
write.table(x = spearman_rhos, file = paste0(dir_out, "avg_exp.tumorcellvaraible_genes.spearman_rho.", run_id, ".tsv"), quote = F, sep = "\t", row.names = T)

# run and write pearson pairwise correlation ------------------------------------------------
pearson_coef <- cor(x = avg.exp.mat[, -1], use="complete.obs", method = "pearson")
pearson_coef
write.table(x = pearson_coef, file = paste0(dir_out, "avg_exp.tumorcellvaraible_genes.pearson_coef.", run_id, ".tsv"), quote = F, sep = "\t", row.names = T)