...
spearman_rhos
write.table(x = spearman_rhos, file = paste0(dir_out, "avg_exp.all_genes.spearman_rho.", run_id, ".tsv"), quote = F, sep = "\t", row.names = T)

# run and write pearson pairwise correlation ------------------------------------------------
pearson_coef <- cor(x = avg.exp.mat[, -1], use="complete.obs", method = "pearson")
pearson_coef
write.table(x = pearson_coef, file = paste0(dir_out, "avg_exp.all_genes.pearson_coef.", run_id, ".tsv"), quote = F, sep = "\t", row.names = T)