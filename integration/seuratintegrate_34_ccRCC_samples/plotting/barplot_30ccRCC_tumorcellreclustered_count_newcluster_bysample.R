...
barcode2cluster_df <- fread(input = "./Resources/Analysis_Results/integration/seuratintegrate_34_ccRCC_samples/annotate_clusterid_on_30ccRCCtumorcellreclustered_byresolution/20220406.v1/ccRCC.34Sample.Tumorcells.Integrated.ReciprocalPCA.Metadata.20220406.v1.tsv", data.table = F)
## input meta data
...
plot_data_df$number_cells_bycluster <- mapvalues(x = plot_data_df$clusterid_new, from = plot_data_df3$clusterid_new, to = as.vector(plot_data_df3$number_cells_bycluster))
...
p <- p + geom_bar(data = plot_data_df, 
                         mapping = aes(x = clusterid_new, y = perc_cells_bysample_eachcluster, fill = sample), stat = "identity")
