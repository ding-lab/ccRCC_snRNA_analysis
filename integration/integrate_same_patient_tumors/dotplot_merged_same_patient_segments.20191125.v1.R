...
# cluster2celltype_tab <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/snRNA_Processed_Data/Cell_Type_Assignment/ccRCC_snRNA_Downstream_Processing - Tumor_Normal.AllCluster2Cell_Type.20191125.v1.tsv", data.table = F)
...
for (case_id_tmp in case_ids) {
  if (case_id_tmp == "C3N-00733") {
    seurat_obj_path <- paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/integration/merge_same_patient_segments/20191125.v1/", case_id_tmp, ".Tumor_Normal.Merged.20191125.v1.RDS")
  }
  if (case_id_tmp == "C3N-01200") {
    seurat_obj_path <- paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/integration/merge_same_patient_segments/20200117.v1/", case_id_tmp, ".Tumor_Normal.Merged.20200117.v1.RDS")
  }
  ...
  p <- DimPlot(seurat_obj, reduction = "umap", label = F, label.size	= 5)
  ...
  file2write <- paste(dir_out, case_id_tmp, ".Tumor_Normal.Dimplot.ByCellType.", run_id, ".png", sep="")
  ...
  p <- p + ggplot2::theme(axis.line=element_blank(),axis.text.x=element_blank(),
                          ...
                          axis.title.y=element_blank(),
                          legend.position = "top")
  file2write <- paste(dir_out, case_id_tmp, ".Tumor_Normal.Dimplot.BySample.", run_id, ".png", sep="")
