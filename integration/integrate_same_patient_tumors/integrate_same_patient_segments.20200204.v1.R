...
saveRDS(object = renal.anchors, file = paste0(dir_out, case_id, ".Tummor_Segments.Anchors.", run_id, ".RDS"))
rm(renal.list)
...
DefaultAssay(renal.integrated) <- "integrated"
...
saveRDS(object = renal.integrated, file = paste0(dir_out, case_id, ".Tummor_Segments.Integrated.", run_id, ".RDS"))


# Dimplot by sample -------------------------------------------------------
p <- DimPlot(renal.integrated, reduction = "umap", group.by = "orig.ident", label = F, label.size	= 5)
p <- p + ggplot2::theme(axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank(),
                        legend.position = "top")
file2write <- paste(dir_out, case_id, ".Tumor_Segments.Integrated.Dimplot.BySample.", run_id, ".png", sep="")
...
## run FindAllMarkers
markers_roc <- FindAllMarkers(object = renal.integrated, test.use = "roc", only.pos = T, return.thresh = 0.5)
...
gene2cellType_tab <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/snRNA_Processed_Data/scRNA_auto/scripts/src/Marker_Gene_Tables/Gene2CellType_Tab.20200203.v1.tsv", data.table = F)
## run FindAllMarkers
markers_wilcox <- FindAllMarkers(object = renal.integrated, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
## annotate cell type marker genes and write as tsv file
