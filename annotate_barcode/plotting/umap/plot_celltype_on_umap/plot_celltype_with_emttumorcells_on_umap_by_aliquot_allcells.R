...
# annotate the cell type to plot ------------------------------------------
barcode2celltype_df <- barcode2celltype_df %>%
  mutate(Cell_group = Cell_group14_w_transitional)

# plot for cell group----------------------------------------------------------
for (id_aliquot_tmp in c("CPT0000880001")) {
...
# for (id_aliquot_tmp in unique(umap_df$aliquot)) {
  ...
  p <- p + geom_point_rast(data = plotdata_df, 
                      ...
                      alpha = 1, size = 0.5)
  # p <- p + scale_color_manual(values = colors_cellgroup14)
...
for (id_aliquot_tmp in c("CPT0000880001")) {
  ...
  pdf(file2write, width = 10, height = 11, useDingbats = F)
  print(p)
  dev.off()
}
