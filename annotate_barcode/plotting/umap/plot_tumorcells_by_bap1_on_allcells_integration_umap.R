...
plot_data_df$Mut.BAP1 <- mapvalues(x = plot_data_df$Id_Aliquot_WU, from = bulkprofile_df$Aliquot_snRNA_WU, to = as.vector(bulkprofile_df$Mut.BAP1))
## filter
plot_data_df <- plot_data_df %>%
  filter(!is.na(Mut.BAP1)) %>%
  filter(Mut.BAP1 != Id_Aliquot_WU) %>%
  mutate(Is.Mut.BAP1 = ifelse(Mut.BAP1 == "None", "WT", "MT"))
## plot
p <- ggplot()
p <- p + geom_point(data = plot_data_df, 
                    mapping = aes(x = UMAP_1, y = UMAP_2, color = Is.Mut.BAP1),
                    alpha = 1, size = 0.05, shape = 16)
p <- p + ggtitle(label = "Tumor Cells By BAP1 Mutation Status") 
...
p
## save as png
file2write <- paste0(dir_out, "BAP1", ".png")
