...
integrated_umap_df <- fread(input = "./Resources/Analysis_Results/integration/31_aliquot_integration/fetch_data/fetch_data_umap/20201104.v1/31_aliquot_integratio..umap_data.tsv", data.table = F)

# plot for cell group----------------------------------------------------------
plot_data_df <- merge(integrated_umap_df %>%
                        mutate(individual_barcode = str_split_fixed(string = barcode_31integration, pattern = "_", n = 2)[,1]),
                      barcode2celltype_df %>%
