...
barcode2celltype_df <- barcode2celltype_df %>%
  mutate(Cell_group = ifelse(Cell_type.detailed %in% c("Mixed myeloid/lymphoid", "CD4/CD8 proliferating"), Cell_type.detailed, Cell_group13))
## map easy ids
...
## sort aliquot id
idmetadata_ordered_df <- idmetadata_df %>%
  filter(Case %in% c("C3N-00733", "C3L-00416", "C3L-00088", "C3N-01200")) %>%
  arrange(Case, desc(Sample_Type), Aliquot.snRNA.WU)
plot_df$Aliquot_WU <- factor(x = plot_df$Aliquot_WU, levels = idmetadata_ordered_df$Aliquot.snRNA.WU)
...
p <- p + theme(strip.text.x = element_text(angle = 90, size = 12),
...
p <- p + theme(axis.text.x = element_text(size = 15, angle = 0, vjust = 0.5),
               axis.ticks.x = element_blank(),
               axis.title = element_blank(),
               axis.text.y = element_text(size = 12))
p
file2write <- paste0(dir_out, "Cell_Group_Composition",".pdf")
