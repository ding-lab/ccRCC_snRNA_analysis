...
srat <- subset(srat, idents = c("Unknown", "Tumor cells"))
# table(srat@meta.data$Cell_type.detailed)
# B-cells                Basophils    CD4/CD8 proliferating             CD4+ T-cells   CD8+ T-cells activated        Endothelial cells 
# 1                        1                        1                       30                        8                        1 
# Macrophages CD204/CD163+       Macrophages VEGFA+   Mixed myeloid/lymphoid           Myofibroblasts          NK cells IL2RB+           NK cells PRF1+ 
#   1                        2                        2                        1                        1                       11 
# Plasma cells                    Tregs              Tumor cells                  Unknown 
# 2                        1                      107                       87
...
p$data$cell_group <- plyr::mapvalues(p$data$id, from = barcode2celltype_filtered_df$Cell_type.detailed, to = barcode2celltype_filtered_df$Cell_group)
cat("###########################################\n")
cat("Dotplot now\n")
p <- p  + RotatedAxis()
