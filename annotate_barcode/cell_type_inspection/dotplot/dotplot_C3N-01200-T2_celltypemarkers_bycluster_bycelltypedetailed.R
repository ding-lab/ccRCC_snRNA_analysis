...
# barcode2celltype_df <- fread(input = "./Resources/Analysis_Results/annotate_barcode/annotate_barcode_with_major_cellgroups/20200910.v1/31Aliquot.Barcode2CellType.20200910.v1.tsv", data.table = F)
## input seurat paths
...
barcode2celltype_filtered_df <- barcode2celltype_df %>%
  ...
  mutate(Cell_group = Cell_type.detailed)
...
count_bycelltype_bycluster_df <- barcode2celltype_filtered_df %>%
  select(Cell_type.shorter, Cell_group, id_seurat_cluster) %>%
  table() %>%
  data.frame() %>%
  filter(Freq > 0) %>%
...
png(file = file2write, width = 3500, height = 1500, res = 150)
print(p)
dev.off()
