...
genes2convert_df <- logFC_wide_df %>%
  filter(!is.na(`PBRM1 mutated vs Non-mutants`) & !is.na(`BAP1 mutated vs Non-mutants`))
...
## get gene list
de_genes <- rowMeans(x = genes2convert_filtered_df[, c("PBRM1 mutated vs Non-mutants", "BAP1 mutated vs Non-mutants")])
de_genes
