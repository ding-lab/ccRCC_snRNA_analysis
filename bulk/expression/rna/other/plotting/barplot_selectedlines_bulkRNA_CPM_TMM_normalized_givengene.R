...
# genes_plot <- c("ABCC3")
genes_plot <- c("SLC6A3")
samples_plot <- c("sample.dr_hk_2_rna", 
                  "sample.dr_rcc_4_vhl_pos_rna",
                  "sample.dr_rcc_4_vhl_neg_rna",
                  "sample.dr_786_o_vhl_neg_rna")
sampletexts_plot <- c("HK-2", "RCC-4-VHL+", "RCC-4-VHL-", "786-O-VHL-")
sampletexts_plot <- c("HK-2", "RCC-4\nVHL+", "RCC-4\nVHL-", "786-O\nVHL-")
...
for (gene_plot in genes_plot) {
  ...
  plot_data_long_df$sample_text <- mapvalues(x = plot_data_long_df$variable, from = samples_plot, to = sampletexts_plot)
  plot_data_long_df$sample_text  <- factor(x = plot_data_long_df$sample_text, levels = sampletexts_plot)
  p <- ggplot()
  ...
  p <- p + theme(axis.text.x = element_text(color = "black", size = 10), # angle = 90, vjust = 0.5, 
                 axis.text.y = element_text(color = "black", size = 12))
  p <- p + theme(axis.title.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 12))
  p
  ...
  png(file2write, width = 350, height = 400, res = 150)
  print(p)
  dev.off()
  
  file2write <- paste0(dir_out, paste0(gene_plot, collapse = "_"), ".bulkRNA.CPM.", "pdf")
  pdf(file2write, width = 3, height = 2, useDingbats = F)
  print(p)
  dev.off()
