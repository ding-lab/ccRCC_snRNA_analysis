...
counts_merged_df <- merge(x = counts1_df %>%
                          ...
                          y = counts2_df %>%
                            dplyr::select(-external_gene_source), by = c("ensembl_gene_id", "entrezgene", "external_gene_name"), suffixes = c(".1", ".2"))
counts_merged_df <- merge(x = counts_merged_df, 
                          y = counts3_df %>%
                            dplyr::select(-external_gene_source), by = c("ensembl_gene_id", "entrezgene", "external_gene_name"))
