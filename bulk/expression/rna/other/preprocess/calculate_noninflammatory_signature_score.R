...
markers_df <- fread(data.table = F, input = "./Resources/Analysis_Results/findmarkers/tumor_subclusters/filter_degs/overlap_intrapatienttumorclusters_inflammatory_top_vs_bottom_with_tumor_markers/20220612.v1/noninflammatory_tumorcell_markers.20220612.v1.tsv")
...
exp_markers_df <- exp_df %>%
  filter(gene_name %in% markers_df$genesymbol_deg)

# process -----------------------------------------------------------------
exp_inflamm_markers_mat <- exp_markers_df[,3:ncol(exp_markers_df)]
...
rownames(exp_inflamm_markers_scale_mat) <- exp_markers_df$gene_name
