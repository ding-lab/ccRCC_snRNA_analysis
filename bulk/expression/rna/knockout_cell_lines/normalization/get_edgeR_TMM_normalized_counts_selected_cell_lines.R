...
counts_mat <- as.matrix(counts_df[, colnames(counts_df)[grepl(pattern = "caki_1_cp|caki_1_control|caki_1_rna|rcc4_", x = colnames(counts_df))]])
...
dir.create(dir_out)
## write output
file2write <- paste0(dir_out, "CPM.TMM_normalized.Knockout_Cell_Lines.", run_id, ".tsv")
write.table(x = countsPerMillion_tmm_df, file = file2write, row.names = F, quote = F, sep = "\t")
file2write <- paste0(dir_out, "Log2CPM.TMM_normalized.Knockout_Cell_Lines.", run_id, ".tsv")
