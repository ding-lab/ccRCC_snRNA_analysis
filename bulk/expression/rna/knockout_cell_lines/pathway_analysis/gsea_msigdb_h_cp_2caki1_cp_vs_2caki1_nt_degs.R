...
## reference: https://www.researchgate.net/figure/Heatmap-of-differentially-expressed-genes-and-GSEA-of-diagnosis-versus-relapse-NPM1mut_fig5_332808042
...
genes_test_df <- genes_background_df %>%
  ...
  arrange(desc(logFC))
nrow(genes_test_df)
geneList_test <- genes_test_df$logFC
names(geneList_test) <- genes_test_df$entrezgene

# do  gene set enrichment analysis and gene set enrichment ------------------------------
enricher_out <- tryCatch(expr = GSEA(geneList = geneList_test, TERM2GENE = gs2gene, pvalueCutoff = 1),
...
enricher_filtered_df <- enricher_out_all_df %>%
