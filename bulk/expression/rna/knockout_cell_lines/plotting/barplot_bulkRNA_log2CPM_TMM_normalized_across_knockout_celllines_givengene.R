...
# input dependencies ------------------------------------------------------
exp_df <- fread(input = "./Resources/Analysis_Results/bulk/expression/rna/knockout_cell_lines/normalization/get_edgeR_TMM_normalized_counts/20220511.v1/Log2CPM.TMM_normalized.Knockout_Cell_Lines.20220511.v1.tsv", data.table = F)
...
for (gene_plot in c("MXI1", "KLF9", "CP", "PCSK6", "HK2", "PKM", "PFKP", "ENO2", "MYC")) {
  ...
  p <- p + theme_classic()
  p <- p + ylab(label = "Log2CPM (TMM normalized)")
  ...
  p
  file2write <- paste0(dir_out, gene_plot, ".bulkRNA.Log2CPM.", "pdf")
  pdf(file2write, width = 2.5, height = 3, useDingbats = F)
  print(p)
  dev.off()
  file2write <- paste0(dir_out, gene_plot, ".bulkRNA.Log2CPM.", "png")
