...
plot_data_long_df$sample_text <- factor(x = plot_data_long_df$sample_text, levels = samples_plot)
plot_data_long_df$external_gene_name <- factor(x = plot_data_long_df$external_gene_name, levels = genes_plot)
p <- ggplot()
p <- p + geom_col(data = plot_data_long_df, mapping = aes(x = sample_text, y = value, fill = external_gene_name), position=position_dodge(), color = "black")
p <- p + theme_classic()
