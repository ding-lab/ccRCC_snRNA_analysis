...
# input dependencies ------------------------------------------------------
exp_df <- fread(input = "./Resources/Analysis_Results/bulk/expression/rna/knockout_cell_lines/divide_knockout_cellline_TPM_TMMnormalized_byparentcelline/20220512.v1/Knockout_celllines.Kallisto.gene_level.CPM.TMMnormalized.DividedByControl.20220512.v1.tsv", data.table = F)
...
samples_plot <- c("rcc4_mxi1_c1", "rcc4_mxi1_c2", "rcc4_scrambled", "rcc4_control",
                  "caki_1_mxi1_c1", "caki_1_control")
genes_plot <- c("CP", "TUBB", "ACTB", "VCL", "TBP", "ATP5J", "PPIA")
genes_plot <- c("CP", "TUBB")
samples_plot <- c("caki_1_control", "caki_1_cp_c1", "caki_1_cp_c2")
...
p <- ggplot()
p <- p + geom_col(data = plot_data_long_df, mapping = aes(x = sample_text, y = value, fill = external_gene_name), position=position_dodge())
p <- p + theme_classic()
