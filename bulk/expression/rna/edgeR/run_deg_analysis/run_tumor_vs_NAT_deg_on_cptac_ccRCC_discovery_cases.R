...
sampleinfo_filtered_df <- sampleinfo_df %>%
  ...
  mutate(condition = ifelse(Sample_Type == "Solid Tissue Normal", "NAT", "Tumor"))
...
sampleGroup <- factor(x = sampleGroup, levels = c("NAT", "Tumor"))
...
# save output -------------------------------------------------------------
file2write <- paste0(dir_out, "Tumor_vs_NAT.glmQLFTest.OutputTables.tsv")
