...
deg_df <- fread(input = "./Resources/Analysis_Results/bulk/expression/edgeR/examine_degs/compile_BAP1_PBRM1_vs_Non-Mutants_DEGs_for_plotting/20210312.v1/BAP1_PBRM1_vs_Non_Mutants.DEGs2pathway.NoDup.20210312.v1.tsv", data.table = F)

# specify the genes to be plotted  -------------------------------------
genes2plot <- unique(deg_df$gene_symbol)
...
# make row split ----------------------------------------------------------
row_split_vec <- mapvalues(x = geneids_plot, from = deg_df$gene_symbol, to = as.vector(deg_df$Description))
...
p <- Heatmap(matrix = plot_data_mat, col = colors_heatmapbody,
             ...
             row_names_gp = gpar(fontsize = 5), row_title_rot = 0,
             right_annotation = row_anno_obj,
             ## column
             column_split = col_split_vec, show_column_dend = F, cluster_column_slices = T, column_title_rot = 90,
             show_column_names = F)
