...
bulk_tumorcontent_df <- readxl::read_excel("./Ding_Lab/Projects_Current/CPTAC/PGDAC/ccRCC_discovery_manuscript/ccRCC Manuscript/CPTAC3-ccRCC-SupplementaryTables_Final/Table S7.xlsx", sheet = "ESTIMATE scores")
## input meta data
...
bulkRNA_tumor_content <- as.numeric(as.vector(as.data.frame(estimate_tab[7,2:176])))
bulkRNA_tumor_content_df <- data.frame(ESTIMATE_TumorPurity_RNA = bulkRNA_tumor_content, Specimen.ID.bulk = unlist(estimate_tab[3,2:176]))
...
merged_tumorcontent_df <- merged_tumorcontent_df %>%
  filter(ESTIMATE_TumorPurity_RNA != Aliquot)
