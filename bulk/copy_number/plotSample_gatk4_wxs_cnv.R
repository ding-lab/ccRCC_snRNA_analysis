...
for (aliquot_tmp in aliquots_all) {
  ...
  ## plotGenome
  filepath_write <- paste0(dir_out, aliquot_tmp, ".WXS_CNV_Somatic.plotSample.", "Chromosomes_Together", ".png")
  png(filename = filepath_write, width = 2000, height = 1500, res = 150)
  plotSample(segments = plotdata_df, ylab = "Log2(TumorCopy/NormalCopy)", seg.lwd = 0.7, ylim = c(-2, 2), layout = c(5,5))
  dev.off()
}