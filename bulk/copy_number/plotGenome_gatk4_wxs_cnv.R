...
for (aliquot_tmp in aliquots_all) {
  ...
  ## plotGenome
  filepath_write <- paste0(dir_out, aliquot_tmp, ".WXS_CNV_Somatic.plotGenome.png")
  png(filename = filepath_write, width = 1000, height = 500, res = 150)
  plotGenome(segments = plotdata_df, ylab = "Log2(TumorCopy/NormalCopy)", seg.lwd = 0.7, ylim = c(-2, 2))
  dev.off()
}