...
##
dir_umap <- "./Resources/snATAC_Processed_Data/Barcode_Annotation/Individual_Sample/TumorClusters/"
filenames_umap <- list.files(path = dir_umap)
# for (aliquot_tmp in "C3L-00416-T1") {
filename_tmp <- filenames_umap[1]
for (filename_tmp in filenames_umap) {
  aliquot_tmp <- str_split_fixed(string = filename_tmp, pattern = "_", n = 3)[,1]
  barcode_df <- fread(data.table = F, input = paste0(dir_umap, filename_tmp))
  plot_data_df <- merge(x = barcode_df, y = cluster2group_df, by.x = c("cell_group"), by.y = c("cluster_name2"), all.x = T)
  ...
  # save plot
  png2write <- paste0(dir_out_png, plot_data_df$sample.y[1], ".png")
