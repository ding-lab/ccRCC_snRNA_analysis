...
# make column order --------------------------------------------------
column_order_vec <- order(scores_epithelial)

# make column split -------------------------------------------------------
column_group_vec <- ifelse(grepl(pattern = "PT", x = columnnames_plot), "PT",
                           ifelse(emt_enriched_vec == "TRUE", "EMT tumor cells"))
