...
# make row split ----------------------------------------------------------
row_split_vec <- mapvalues(x = genes_plot, from = emt_genes_df$Gene, to = as.vector(emt_genes_df$Gene_Group2))
row_split_vec
row_split_factor <- factor(x = row_split_vec, levels = c("Mesenchymal", "Epithelial", "Proximal tubule", "Tumor cells"))
