...
# summary(scores_s12)
colors_s12score <- colorRamp2(c(-50, 0, 50), 
                              c("white", brewer.pal(n = 9, name = "BuPu")[c(3,8)]))
colors_s3score <- colorRamp2(c(-50, 0, 50), 
                              c("white", brewer.pal(n = 11, name = "PiYg")[c(5,2)]))
...
colanno_df <- data.frame(columnname = columnnames_plot,
                         ...
                         cell_group = ifelse(scores_epithelial >= quantile(x = scores_epithelial, 0.7), "Epithelial-strong",
...
colanno_obj = HeatmapAnnotation(#link = anno_mark(at = index_highlight, labels = texts_highlight, labels_gp = gpar(fontsize = 15), side = "top"),
                                # EpithelialEnriched = anno_simple(x = epi_enriched_vec, col = colors_emtenriched[epi_enriched_vec], height = unit(1, "cm")),
                                ...
                                PT_S3_Score = anno_simple(x = scores_s3, col = colors_s12score, height = unit(0.75, "cm")),
...
column_group_vec <- colanno_df$cell_group
