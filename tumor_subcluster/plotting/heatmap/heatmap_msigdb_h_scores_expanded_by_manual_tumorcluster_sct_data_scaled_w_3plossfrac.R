...
scores_df <- fread(data.table = F, input = "./Resources/Analysis_Results/tumor_subcluster/calculate_scores/calculate_msigdb_top_geneset_scores/20210525.v1/MSigDB.Hallmark.tsv")
...
## get dim names
# scorenames <-  colnames(enrich_df)[1:20]
scorenames <-  colnames(scores_df)[1:21]
...
col_order_df <- enrich_df %>%
  dplyr::arrange(desc(Cell_cycle), desc(Immune), desc(EMT), desc(mTOR))
...
colanno_df <- colanno_df[colnames(plot_data_mat),-1]
## make colors
...
pdf(file2write, width = 9, height = 7, useDingbats = F)
draw(object = p,
     annotation_legend_side = "top", annotation_legend_list = list_lgd)
dev.off()
