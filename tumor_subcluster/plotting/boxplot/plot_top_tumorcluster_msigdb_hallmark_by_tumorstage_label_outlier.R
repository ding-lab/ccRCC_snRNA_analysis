...
for (scoregroup_tmp in unique(scores_df$score_group)) {
  ...
  plot_data_df <- plot_data_df %>%
    ...
    mutate(y_plot = value)
  outlier_df <- plot_data_df %>%
    group_by(x_plot) %>%
    summarise(iqr = IQR(value), outlier_low = quantile(x = value, probs = 0.25) - 1.5*iqr, outlier_high = quantile(x = value, probs = 0.75) + 1.5*iqr)
  plot_data_df <- merge(x = plot_data_df, y = outlier_df, by = c("x_plot"), all.x = T)
  plot_data_df <- plot_data_df %>%
    mutate(is_outlier = (value > outlier_high) | (value < outlier_low))
  ## plot
  ...
  p <- p + geom_text_repel(data = subset(plot_data_df, subset = is_outlier), aes(x=x_plot, y=y_plot, label = Aliquot.snRNA.WU), position = pos)
