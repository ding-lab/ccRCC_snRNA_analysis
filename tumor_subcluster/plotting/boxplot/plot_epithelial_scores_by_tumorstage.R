...
scores_df <- fread(data.table = F, input = "./Resources/Analysis_Results/tumor_subcluster/calculate_scores/calculate_Epithelial_scores_EMTmoduledown_wPT/20210908.v1/EpithelialScore.tsv")
...
file2write <- paste0(dir_out, gsub(x = "Epithelial.", pattern = "\\/", replacement = "_"), ".3groups.", ".pdf")
pdf(file2write, width = 3, height = 2.5, useDingbats = F)
print(p)
dev.off()

# plot by 2 stage groups ------------------------------------------------------
plot_data_df <- idmetadata_df %>%
  filter(Sample_Type == "Tumor") %>%
  filter(Case != "C3L-00359") %>%
  filter(snRNA_available == TRUE) %>%
...
p <- p + theme(legend.position = "none")
# p <- p + ylab(label = scoregroup_tmp)
p <- p + ylab(label = "Epithelial score")
p <- p + theme(axis.title.x = element_blank(), axis.title.y = element_text(size = 10), axis.text = element_text(color = "black"))

file2write <- paste0(dir_out, gsub(x = "Epithelial.", pattern = "\\/", replacement = "_"), ".2groups.", ".pdf")
pdf(file2write, width = 3, height = 2.5, useDingbats = F)
print(p)
dev.off()
