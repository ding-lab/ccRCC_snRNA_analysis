...
plot_data_long_df <- avgexp_df %>%
  ...
  filter(id_bycluster_byaliquot %in% cluster_pass_df$colname_exp | grepl(pattern = "PT", x = id_bycluster_byaliquot)) %>%
...
file2write <- paste0(dir_out, "Gene2Pathway.tsv")
write.table(x = gene2pathway_df, file = file2write, quote = F, sep = "\t", row.names = F)
file2write <- paste0(dir_out, "Genes.tsv")
write.table(x = gene2pathway_df, file = file2write, quote = F, sep = "\t", row.names = F)
