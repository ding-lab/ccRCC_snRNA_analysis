...
for (module_tmp in unique(modules_df$module_name)) {
  ...
  scores_tmp_df <- scores_wide_df[, colnames_tmp]; rownames(scores_tmp_df) <- scores_wide_df$cluster_name
  ...
  if (module_tmp == "mTOR") {
    cutoffs_tmp[grepl(x = names(cutoffs_tmp), pattern = "mTOR", ignore.case = T)] <- quantile(x = scores_tmp_df[,"MTORC1_SIGNALING_Score"], probs = 0.9)
  }
  scores_pass_df <- data.frame(scores_tmp_df > cutoffs_tmp)
