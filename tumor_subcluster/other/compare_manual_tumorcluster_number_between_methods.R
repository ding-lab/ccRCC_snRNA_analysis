...
barcode2manualcluster_ds2000_df <- fread(data.table = F, input = "./Resources/Analysis_Results/annotate_barcode/map_tumorsubclusterid/map_barcode_with_manual_tumorsubcluster_id_downsample2000cells/20220307.vcutoff25cells/Barcode2TumorSubclusterId.20220307.vcutoff25cells.tsv")
...
mean(abs(count_merged_df$Freq.orig - count_merged_df$Freq.ds2000), na.rm = T)
mean(abs(count_merged_df$Freq.orig - count_merged_df$Freq.pc50))

# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Tumorsubclusternumber.3methods.", run_id, ".tsv")
