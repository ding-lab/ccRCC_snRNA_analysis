...
variable_genes_df <- fread(input = "./Resources/Analysis_Results/findvariablefeatures/findvariablefeatures_sct_Lake_bycluster_katmai/20210819.v1/findvariablefeatures.Lake_etal.20210819.v1.tsv", data.table = F)
...
data_col_names.filtered <- data_col_names.changed[grepl(pattern = "_Fibroblasts", x = data_col_names.changed)]
...
pearson_coef
write.table(x = pearson_coef, file = paste0(dir_out, "pearson_coef.", "stromavaraible_genes.", "fibroblasts.", "avg_exp.", "sct.", "byaliquot.", run_id, ".tsv"), quote = F, sep = "\t", row.names = T)
