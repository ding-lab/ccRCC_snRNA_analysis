...
pearson_coef.tumorcellvariable_genes.df <- fread(input = "./Resources/Analysis_Results/pairwise_correlation/calculate_stroma_pairwise_correlation_sct_stromacellvariable_genes/20200901.v1/pearson_coef.stromavaraible_genes.stroma.avg_exp.sct.byaliquot.20200901.v1.tsv", data.table = F)
...
number2rangetext = function(x) {
  if (x < 200) {
    range_low <- floor(x/40)*40
    range_high <- ceiling((x+1)/40)*40
...
names(colors_numbercellrange_vec) <- sapply(X = seq(from = 1, to = 201, by = 40), FUN = number2rangetext)
...
## save heatmap as pdf
pdf(file = paste0(dir_out, "avg_exp.stromacellvariable_genes.pearson_coef.heatmap",".pdf"), 
    width = 20, height = 16)
draw(object = p, 
     annotation_legend_side = "right", annotation_legend_list = annotation_lgd)
dev.off()
