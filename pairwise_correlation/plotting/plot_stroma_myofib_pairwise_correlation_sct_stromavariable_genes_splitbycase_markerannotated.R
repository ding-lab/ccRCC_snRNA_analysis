...
## input marker gene expression
exp_anno_df <- fread(data.table = F, input = "./Resources/Analysis_Results/average_expression/extract_expression/extract_markergenes_myofibroblasts_averaged_bysample/20200901.v1/myofibroblasts_markergenes.myofibroblastsbysample..tsv")
## specify cell type
...
exp_anno_mat <- t(exp_anno_df[, aliquot_celltype])
colnames(exp_anno_mat) <- exp_anno_df$V1
...
## make color function for heatmap body colors
col_fun_exp = colorRamp2(c(quantile(x = exp_anno_mat, probs = 0.1, na.rm = T), 
                       quantile(x = exp_anno_mat, probs = 0.5, na.rm = T), 
                       quantile(x = exp_anno_mat, probs = 0.9, na.rm = T)), 
                     c("white", "pink", "purple"))
...
col_anno = HeatmapAnnotation(Sample_Type_Suffix = anno_simple(x = suffixes_aliquot_id,
                             ...
                             Number_Cells = anno_simple(x = number_cells_byaliquot$text_range,
                                                        col = colors_numbercellrange_vec,
                                                        height = unit(7, "mm")),
                             CALD1_snRNA = anno_simple(x = exp_anno_mat[, "CALD1"], col = col_fun_exp),
                             ACTA2_snRNA = anno_simple(x = exp_anno_mat[, "ACTA2"], col = col_fun_exp),
                             TAGLN_snRNA = anno_simple(x = exp_anno_mat[, "TAGLN"], col = col_fun_exp),
                             CACNA1C_snRNA = anno_simple(x = exp_anno_mat[, "CACNA1C"], col = col_fun_exp),
                             TNC_snRNA = anno_simple(x = exp_anno_mat[, "TNC"], col = col_fun_exp),
                             PDGFRB_snRNA = anno_simple(x = exp_anno_mat[, "PDGFRB"], col = col_fun_exp),
                             COL1A1_snRNA = anno_simple(x = exp_anno_mat[, "COL1A1"], col = col_fun_exp),
                             COL5A1_snRNA = anno_simple(x = exp_anno_mat[, "COL5A1"], col = col_fun_exp),
                             COL12A1_snRNA = anno_simple(x = exp_anno_mat[, "COL12A1"], col = col_fun_exp),
                             COL3A1_snRNA = anno_simple(x = exp_anno_mat[, "COL3A1"], col = col_fun_exp),
                             # Histologic_Type = anno_simple(x = omics_long_df$Histologic_Type,
                             #                               col = colors_hist_type,
                             #                               width = unit(1.5, "cm")),
...
annotation_lgd = list(
  ...
  Legend(col_fun = col_fun_exp, 
         title = paste0("snRNA Expression"), 
         direction = "vertical"))
## save heatmap as png
png(filename = paste0(dir_out, "heatmap", ".png"), 
    width = 3000, height = 2600, res = 150)
draw(object = p, 
