...
top_col_anno = HeatmapAnnotation(Sample_Type_Suffix = anno_text(aliquot_id_suffix, 
                                 ...
                                 snATAC_Seq = anno_simple(x = as.character(top_col_anno_df$snATAC_available),
                                                          simple_anno_size = unit(4, "mm"), 
                                                          gp = gpar(color = "black"), col = colors_sn_data),
                                 WES = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                   simple_anno_size = unit(4, "mm"), 
                                                   gp = gpar(color = "black"), col = colors_bulk_data),
                                 WGS = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                   simple_anno_size = unit(4, "mm"), 
                                                   gp = gpar(color = "black"), col = colors_bulk_data),
                                 Methylation = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                           simple_anno_size = unit(4, "mm"), 
                                                           gp = gpar(color = "black"), col = colors_bulk_data),
                                 RNA_Seq = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                       simple_anno_size = unit(4, "mm"), 
                                                       gp = gpar(color = "black"), col = colors_bulk_data),
                                 Protein = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                       simple_anno_size = unit(4, "mm"), 
                                                       gp = gpar(color = "black"), col = colors_bulk_data),
                                 Phosphoprotein = anno_simple(x = as.character(!is.na(top_col_anno_df$Aliquot.bulk)),
                                                              simple_anno_size = unit(4, "mm"), 
                                                              gp = gpar(color = "black"), 
                                                              col = colors_bulk_data),
                                 Histology_HE = anno_simple(x = as.character(top_col_anno_df$snRNA_available),
                                                            simple_anno_size = unit(4, "mm"), 
                                                            gp = gpar(color = "black"), col = colors_bulk_data),
                                 FFPE_Slides = anno_simple(x = as.character(top_col_anno_df$FFPE_available_slides > 0),
                                                           simple_anno_size = unit(4, "mm"), 
                                                           gp = gpar(color = "black"), col = colors_bulk_data),
                                 ...
                                 Translocation.other = anno_simple(x = as.character(top_col_anno_df$Translocation.t3_other != "None"),
                                                                   gp = gpar(color = "black"),
                                                                   simple_anno_size = unit(3, "mm"),
...
annotation_lgd = list(
  ...
  Legend(labels = c("With Chr3 Translocation", "None", "No Data"),
         title = "Bulk Chr3 Translocation",
         legend_gp = gpar(fill = c("black", "white", color_na))),
