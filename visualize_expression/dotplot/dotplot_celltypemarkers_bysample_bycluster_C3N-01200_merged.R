...
## map cell type
srat@meta.data$Cell_group.detailed <- mapvalues(x = srat@meta.data$id_cell, from = barcode2celltype_df$id_cell, to = barcode2celltype_df$Cell_group.detailed)
Idents(srat) <- "Cell_group.detailed"
## subset
srat <- subset(x = srat, idents = c("Normal epithelial cells", "Tumor cells"))
## add 
...
srat@meta.data$Id_Cluster_Sample <- paste0("C", srat@meta.data$seurat_clusters, '_', srat@meta.data$Aliquot_WU)
Idents(srat) <- "Id_Cluster_Sample"
...
png(file = file2write, width = 6000, height = 2000, res = 150)
print(p)
dev.off()

# plot not scaled -------------------------------------------------------------
p <- DotPlot(object = srat, features = genes2plot_filtered, col.max = 7, scale = F, assay = "RNA")
p$data$gene_cell_type_group <- plyr::mapvalues(p$data$features.plot, from = gene2celltype_df$Gene, to = gene2celltype_df$Cell_Type_Group)
p$data$gene_cell_type1 <- plyr::mapvalues(p$data$features.plot, from = gene2celltype_df$Gene, to = gene2celltype_df$Cell_Type1)
p$data$gene_cell_type2 <- plyr::mapvalues(p$data$features.plot, from = gene2celltype_df$Gene, to = gene2celltype_df$Cell_Type2)
p$data$gene_cell_type3 <- plyr::mapvalues(p$data$features.plot, from = gene2celltype_df$Gene, to = gene2celltype_df$Cell_Type3)
