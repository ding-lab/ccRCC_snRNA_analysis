...
gene2celltype_df <- fread(data.table = F, input = "./Resources/Analysis_Results/dependencies/write_emt_genes/20200915.v1/EMT_Genes.20200915.v1.tsv")
...
# specifgy genes to filter ------------------------------------------------
genes_filter <- gene2celltype_df$hgnc_symbol
...
# for (aliquot2process in unique(paths_srat_df$Aliquot)) {
  ...
  genes2plot <-  intersect(genes_filter, srat@assays$RNA@counts@Dimnames[[1]])
  ...
  ## add facet
  plotdata_df$gene_group1 <- plyr::mapvalues(plotdata_df$features.plot, from = gene2celltype_df$hgnc_symbol, to = gene2celltype_df$gene_function)
  ...
  plotdata_df$cell_group <- plyr::mapvalues(plotdata_df$cell_type, from = barcode2celltype_filtered_df$Cell_group14, to = barcode2celltype_filtered_df$Cell_group3)
  ...
  p <- p + facet_grid(cell_group~gene_group1, scales = "free", space = "free", drop = T)
  ...
  png(file = file2write, width = 3000, height = 700, res = 150)
  print(p)
  dev.off()
}
