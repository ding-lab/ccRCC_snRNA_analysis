...
p <- p + geom_dotplot(data = plotdata_df, mapping = aes(x = y_plot, y = x_plot, fill = data_type), binaxis='y', stackdir='center', position=position_dodge(0.8), alpha = 0.7)
...
p <- p + guides(fill = guide_legend(override.aes = list(size=3), nrow = 3, title = NULL))
