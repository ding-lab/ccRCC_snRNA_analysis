...
plotdata_wide_df <- genes_process_df %>%
  ...
  select(Gene, avg_log2FC.allTumorcellsvsPT, log2FC.bulkRNA, log2FC.bulkpro, log2FC.snATAC) %>%
...
plotdata_df <- plotdata_df %>%
  mutate(data_type = ifelse(variable == "avg_log2FC.allTumorcellsvsPT", dataname_snrna,
                            ifelse(variable == "avg_log2", dataname_snatac, ifelse(variable == "log2FC.bulkRNA", dataname_bulk_rna, dataname_bulk_protein)))) %>%
