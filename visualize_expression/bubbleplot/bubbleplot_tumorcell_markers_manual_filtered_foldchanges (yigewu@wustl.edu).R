...
plotdata_df <- plotdata_df %>%
  mutate(data_type = ifelse(variable == "avg_log2FC.allTumorcellsvsPT", "Tumor cells vs. non-tumor cells (snRNA-seq)",
                            ifelse(variable == "log2FC.bulkRNA", "Tumors vs. NATs (bulk RNA-seq)", "Tumors vs. NATs (bulk proteomics)"))) %>%
  mutate(x_plot = value) %>%
  mutate(y_plot = Gene)
## make colors
display.brewer.all()
...
# plot --------------------------------------------------------------------
p <- ggplot(data = plotdata_df, mapping = aes(x = y_plot, y = x_plot, fill = data_type))
...
p <- p + guides(fill = guide_legend(override.aes = list(size=3), nrow = 3))
file2write <- paste0(dir_out, "Foldchanges", ".png")
png(file2write, width = 650, height = 900, res = 150)
print(p)
dev.off()
