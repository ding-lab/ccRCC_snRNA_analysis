...
# color_orange <- RColorBrewer::brewer.pal(n = 11, name = "PuOr")[2]
# color_purple <- RColorBrewer::brewer.pal(n = 11, name = "PuOr")[10]
...
# color_brown <- RColorBrewer::brewer.pal(n = 11, name = "BrBG")[2]
# color_green <- RColorBrewer::brewer.pal(n = 11, name = "BrBG")[10]
...
index_highlight <- which(columnnames_plot %in% c("C3L-00079-T1_7_Transitional.cells"))
...
emt_group_vec <- (scores_mesenchymal >= quantile(x = scores_mesenchymal, probs = 0.9)) & (scores_epithelial <= quantile(x = scores_epithelial, probs = 0.1))
