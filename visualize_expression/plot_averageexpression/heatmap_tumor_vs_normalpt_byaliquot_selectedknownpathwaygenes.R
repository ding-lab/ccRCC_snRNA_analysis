...
# set plot gene list ------------------------------------------------------
genes_plot <- c("HIF1A", "EPAS1", "VHL", hiftargets_df$target_genesymbol, 
                pbaf_genes, "SETD2", "BAP1", "KDM5C", 
                REACT[["165159	mTOR signalling"]])
genes_plot <- intersect(genes_plot, deg_summary_df$gene_symbol[deg_summary_df$Freq >= 4])
genes_plot
...
p <- Heatmap(matrix = plot_data_mat,
             ...
             # cluster_rows = F, 
             row_split = c(rep("HIF", 15), rep("Epigenetic", 2), rep("MTOR", 4)),
