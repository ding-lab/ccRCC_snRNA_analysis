...
# set plot gene list ------------------------------------------------------
genes_plot <- tf_tab$target_genesymbol[tf_tab$source_genesymbol == "RORA"]
genes_plot
# genes_plot <- c(genes_plot, "RORA", "VEGFA", "HIF1A", "NR3C1", "STAT3", "PFKFB3", "TFRC", "EGLN3", "POU5F1")
genes_plot <- c("HIF1A", "SP1", "SP3", "RORA", 
                "ARNTL", "CLOCK", 
                "APOC3", "APOA5", "APOA1", 
                "PCK1", "G6PC",
                "SPARC", "PLG", "AGRP",
                "SOX4", "TP53", "CDKN1A", "CCNA1", "MYCN",
                "NR1D1", "NR1D2")
genes_plot
...
top_col_anno <- HeatmapAnnotation(Cell.Type = anno_simple(x = ids_celltype, col = colors_celltype),
                                  Bulk.Protein.RORA = anno_simple(x = unlist(protein_selectedgenes_mat["RORA",])))
...
p <- Heatmap(matrix = plot_data_mat,
             ...
             right_annotation = row_anno, cluster_rows = F,
...
# save output -------------------------------------------------------------
file2write <- paste0(dir_out, "rora_pathway.", "expression.", "tumor_vs_normal_proximal_tubule.", run_id, ".png")
