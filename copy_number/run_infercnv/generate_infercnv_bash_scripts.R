...
baseD = "~/Box/"
setwd(baseD)
source("./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/ccRCC_snRNA_analysis/load_pkgs.R")
## set run id
run_id <- "20200413.v1"
...
paths_srat <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/individual_sample/write_path_to_seurat_objects_on_box/20200219.v1/Seurat_Object_Paths.20200219.v1.tsv", data.table = F)
...
for (snRNA_aliquot_id_tmp in unique(paths_srat$Aliquot)) {
  ...
  if (!file.exists(path_infercnv_script)) {
    ...
    ref_anno_tab <- anno_tab %>%
      filter(!(infercnv_group %in% c("Tumor.cells", "Unknown")))
    ref_infercnv_group <- unique(ref_anno_tab$infercnv_group)
