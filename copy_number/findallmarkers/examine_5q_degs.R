...
ora_result <- fread(input = "Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/copy_number/findallmarkers/examine_5q_degs/20200302.v1/ORA_results.tab", data.table = F)
...
write.table(x = deg_by_aliquot_count, file = paste0(dir_out, chr_region_tmp, ".FindMarkers.Wilcox.ExpectedCNV_vs_Neutral", ".Count.", run_id, ".tsv"), sep = "\t", quote = F, row.names = F)

# annotate gene to TFs -----------------------------------------------
## filter by gene
...
write.table(x = deg_filtered_by_gene, file = paste0(dir_out, chr_region_tmp, ".FindMarkers.Wilcox.ExpectedCNV_vs_Neutral", ".TF.", run_id, ".tsv"), sep = "\t", quote = F, row.names = F)
...
write.table(x = deg_by_chr_region_df, file = paste0(dir_out, "5q.FindMarkers.Wilcox.ExpectedCNV_vs_Neutral", ".", run_id, ".tsv"), sep = "\t", quote = F, row.names = F)
...
write.table(x = gene2path_df, file = paste0(dir_out, "5q.FindMarkers.Wilcox.ExpectedCNV_vs_Neutral",  ".", "Gene2Pathway",".", run_id, ".tsv"), sep = "\t", quote = F, row.names = F)
