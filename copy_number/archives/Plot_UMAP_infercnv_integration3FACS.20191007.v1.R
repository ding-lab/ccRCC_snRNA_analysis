...
tumor_cnv_state_mat <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/outputs/integration.20190927.v1/CPT0086350004/infercnv.14_HMM_predHMMi6.hmm_mode-samples.Pnorm_0.5.repr_intensities.observations.txt", data.table = F)
ref_cnv_state_mat <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/outputs/integration.20190927.v1/CPT0086350004/infercnv.14_HMM_predHMMi6.hmm_mode-samples.Pnorm_0.5.repr_intensities.references.txt", data.table = F)
...
for (gene_tmp in genes2plot) {
  if (gene_tmp %in% cnv_state_df$V1) {
    ...
    file2write <- paste(dir_out, sample_id,".", gene_tmp, ".FeaturePlot_CNA.", run_id, ".pdf", sep="")
