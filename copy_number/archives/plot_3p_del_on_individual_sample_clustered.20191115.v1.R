...
for (snRNA_aliquot_id_tmp in snRNA_aliquot_ids) {
  ...
  gene_tmp1 <- genes2plot[1]
  gene_tmp2 <- genes2plot[2]
  infercnv_observe_gene1_tab <- cnv_state_df %>%
    rename(gene_symbol = V1) %>%
    filter(gene_symbol == gene_tmp1) %>%
  ...
  infercnv_observe_gene2_tab <- cnv_state_df %>%
    rename(gene_symbol = V1) %>%
    filter(gene_symbol == gene_tmp2) %>%
    mutate(barcode = str_split_fixed(string = variable, pattern = "_", n = 2)[,1]) %>%
    rename(copy_state = value) %>%
    select(gene_symbol, barcode, copy_state)
  infercnv_observe_gene_tab <- merge(infercnv_observe_gene1_tab, infercnv_observe_gene2_tab, by = c("barcode"), all = T, suffixes = c(".1", ".2"))
  infercnv_observe_gene_tab$cnv_cat <- ifelse(infercnv_observe_gene_tab$copy_state.1 < 1.0 & infercnv_observe_gene_tab$copy_state.2 < 1.0, paste0("Loss of ", gene_tmp1, " & ", gene_tmp2),
                                              ifelse(infercnv_observe_gene_tab$copy_state.1 < 1.0 & infercnv_observe_gene_tab$copy_state.2 == 1.0, paste0("Loss of ", gene_tmp1, " only"),
                                                     ifelse(infercnv_observe_gene_tab$copy_state.1 == 1.0 & infercnv_observe_gene_tab$copy_state.2 < 1.0, paste0("Loss of ", gene_tmp2, " only"),
                                                            ifelse(infercnv_observe_gene_tab$copy_state.1 == 1.0 & infercnv_observe_gene_tab$copy_state.2 == 1.0, paste0("CN Neutral"), "Other"))))
  table(infercnv_observe_gene_tab$cnv_cat)
  tab2p <- umap_tab
