  ...
  for (gene_tmp in genes2plot) {
    if (gene_tmp %in% cnv_state_df$V1) {
      ...
      p <- ggplot() +
        ...
        ggtitle(paste0(snRNA_aliquot_id_tmp, " Malignant Cells Only\n", gene_tmp, "_Copy_Number_Status"))
      ...
      file2write <- paste(dir_out, snRNA_aliquot_id_tmp,".Individual_Malignant_Reclustered.", gene_tmp, ".FeaturePlot_CNA.", run_id, ".png", sep="")
...
for (gene_tmp in c("VHL", "PBRM1")) {
  ...
  p <- DotPlot(seurat_obj2, features = genes2plot, group.by = "Cluster_cna_cat", col.max = 2.5, col.min = 0) + RotatedAxis()
