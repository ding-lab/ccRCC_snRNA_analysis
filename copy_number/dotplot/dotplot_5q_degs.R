...
genetic_alt_downstream_genes <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/dependencies/write_ccrcc_genetic_event_downstream_genes/20200302.v1/ccRCC_Genetic_Event_Downstream_Genes.20200302.v1.tsv", data.table = F)
## set chr_region to process
chr_region_tmp <- "5q"

# plot all the recurrent degs ----------------------------------------
## get the biologically meaningful genes to filer
genes2filter <- genetic_alt_downstream_genes$target_genesymbol[genetic_alt_downstream_genes$source_genesymbol == "NFE2L2"]
genes2filter
...
## set them for the facet
p <- p + theme(strip.text.y = element_text(angle = 0),
               panel.spacing = unit(0, "lines"))
p
png(filename = paste0(dir_out, chr_region_tmp, "_Recurrent_DEG_Expression.", run_id, ".png"), width = 1000, height = 500)
print(p)
dev.off()
