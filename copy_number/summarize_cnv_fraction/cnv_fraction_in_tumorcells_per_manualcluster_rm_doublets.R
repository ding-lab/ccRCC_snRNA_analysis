...
barcode2cluster_df <- fread(input = "./Resources/Analysis_Results/annotate_barcode/map_tumorsubclusterid/map_barcode_with_manual_tumorsubcluster_id/20210805.v1/Barcode2TumorSubclusterId.20210805.v1.tsv", data.table = F)
## input doublet info
...
for (infercnv_run_id in c("Individual.20200305.v1", "run.20210805")) {
  ...
  for (aliquot_tmp in aliquots2process) {
    ...
    ## get the manual cluster info for this aliquot
    ...
    ## get doublet info for this aliquot
    ...
    cnv_state_mat.m <- cnv_state_mat.m %>%
      ...
      filter(!(barcode %in% scrublets_df$Barcodes)) %>%
...
cnv_3state_count_aliquots <- tmp %>%
  ...
  summarise(Fraction = sum(Fraction, na.rm = T), Freq = sum(Freq, na.rm = T), num_cells_nonna = unique(num_cells_nonna))
## write
