...
tumorcell2cnv_bychr_df <- merge(tumorcell2cnv_bychr_df, barcode2manualsubcluster_df, 
                                by.x = c("aliquot", "individual_barcode"), by.y = c("orig.ident", "barcode"), all.x = T)
## make manual subcluster name
tumorcell2cnv_bychr_df <- tumorcell2cnv_bychr_df %>%
  mutate(manual_cluster_name = paste0(aliquot, "_MC", manual_cluster_id))

# calculate the fraction of tumor cells with expected cnv -----------------
## group by manual_subcluster_id and aggregate the number of barcodes with expected cnv
...
count_expectedcnv_bychr_bycluster_df <- aggregate(x = tumorcell2isexpectedcnv_bychr_mat, list(tumorcell2cnv_bychr_df$manual_cluster_name), sum)
count_expectedcnv_bychr_bycluster_df <- count_expectedcnv_bychr_bycluster_df %>%
  rename(manual_cluster_name = Group.1)
## group by sample and aggregate the number of total barcodes
tumorcell2iscnv_bychr_mat <- (!is.na(tumorcell2cnv_bychr_mat)) + 0
count_cnv_bychr_bycluster_df <- aggregate(x = tumorcell2iscnv_bychr_mat, list(tumorcell2cnv_bychr_df$manual_cluster_name), sum)
count_cnv_bychr_bycluster_df <- count_cnv_bychr_bycluster_df %>%
  rename(manual_cluster_name = Group.1)

# write outputs -------------------------------------------
## write the number cells with expected cnv
file2write <- paste0(dir_out, "number_of_tumorcells.expectedCNA.by_chr_region.by_manual_subcluster.", run_id, ".tsv")
write.table(x = count_expectedcnv_bychr_bycluster_df, file = file2write, quote = F, sep = "\t", row.names = F)
## write the number cells with cnv calls
file2write <- paste0(dir_out, "number_of_tumorcells.withCNAcalls.by_chr_region.by_manual_subcluster.", run_id, ".tsv")
write.table(x = count_cnv_bychr_bycluster_df, file = file2write, quote = F, sep = "\t", row.names = F)
## write the number cells with cnv calls
frac_expectedcnv_bychr_bycluster_df <- count_expectedcnv_bychr_bycluster_df[, chr_regions2process]/count_cnv_bychr_bycluster_df[,chr_regions2process]
frac_expectedcnv_bychr_bycluster_df <- cbind(data.frame(manual_cluster_name = count_expectedcnv_bychr_bycluster_df$manual_cluster_name),
                                             frac_expectedcnv_bychr_bycluster_df)
file2write <- paste0(dir_out, "fraction_of_tumorcells.expectedCNA.by_chr_region.by_manual_subcluster.", run_id, ".tsv")
write.table(x = frac_expectedcnv_bychr_bycluster_df, file = file2write, quote = F, sep = "\t", row.names = F)
