...
for (snRNA_aliquot_id_tmp in seurat_summary2process$Aliquot) {
  ...
  ## get umap coordates
  barcode_df <- FetchData(seurat_object, vars = c("orig.ident", "ident"))
  ...
  for (chr_region_tmp in unique(ccrcc_cna_genes_df$chr_region)) {
    ...
    for (gene_tmp in genes2plot) {
      file2write <- paste(dir_out2, snRNA_aliquot_id_tmp, ".nephron_epithelium_reclustered.", "20200217.v1",
                          ".barplot_frac_cells_with_cnv.", gene_tmp, ".", run_id, ".png", sep="")
      if ((gene_tmp %in% cnv_state_df$V1) & !file.exists(file2write)) {
        ...
        ## get the fraction of copy number loss or gain depending on the gene
        if (ccrcc_cna_genes_df$gene_cna_type[ccrcc_cna_genes_df$gene_symbol == gene_tmp] == "Loss") {
