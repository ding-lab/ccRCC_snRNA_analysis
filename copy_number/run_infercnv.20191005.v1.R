...
nrow(anno_tab)
write.table(x = anno_tab, file = paste0(dir_out, sample_id, ".Barcode_Annotation.", run_id, ".txt"), quote = F, sep = "\t", row.names = F, col.names = F)
...
# run infercnv ------------------------------------------------------------
sink(file = paste0(dir_out, sample_id, ".inferCNV_processing.", run_id, ".txt"))
infercnv_obj = infercnv::CreateInfercnvObject(raw_counts_matrix=clean_exp_mat,
                                    annotations_file=paste0(dir_out, sample_id, ".Barcode_Annotation.", run_id, ".txt"),
                                    delim="\t",
                                    gene_order_file= paste0("./Ding_Lab/Projects_Current/RCC/ccRCC_single_cell/Resources/snRNA_Processed_Data/inferCNV/inputs/gencode_v21_gen_pos.MatchCellRangerFeatures.NoDuplicates.", run_id,".txt"),
                                    ref_group_names= ref_group_names)
saveRDS(object = infercnv_obj, file = paste0(dir_out, sample_id, ".infercnv_obj.", run_id, ".RDS"))
