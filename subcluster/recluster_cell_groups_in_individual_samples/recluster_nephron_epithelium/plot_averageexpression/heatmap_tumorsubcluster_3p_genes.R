...
avg.exp.mat <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_snRNA/Resources/Analysis_Results/integration/30_aliquot_integration/averageexpression/averageexpression_tumor_cells_by_tumor_exp_subcluster/20200311.v1/averageexpression_tumor_cells_by_tumor_exp_subcluster.20200311.v1.tsv", data.table = F)
...
plot_genes_tmp <- as.vector(ccrcc_gene_alt_downstream_genes_df$target_genesymbol[ccrcc_gene_alt_downstream_genes_df$source_genesymbol %in% c("HIF1A", "EPAS1")])
...
## save heatmap
png(filename = paste0(dir_out, "avg_exp.HIF_targets.by_tumor_exp_subcluster.heatmap.", run_id, ".png"), 
